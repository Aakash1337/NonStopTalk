...
- In-progress web platform: versioned Worker APIs, central D1 repositories/migrations, an internal sync-profile foundation, scheduled anonymous-data expiry, coarse D1/Analytics Engine event aggregation, a schema-v6 milestone receiver with active receipt expiry cleanup and a dormant pre-existing-outbox rollback consumer, and aggregate D1 model-usage budgeting
...
The platform slice keeps composition in `cloudflare/worker.ts` and separates live room coordination, relational storage, coaching backup, identity, maintenance health, analytics, and model providers behind small modules. D1 is the central queryable store for anonymous device expiry, consented compact summaries, consent records, HMAC-pseudonymous room facts, one retention-cleanup heartbeat, best-effort daily analytics counters, schema-v6 milestone receipts for the internal receiver, and aggregate model-usage budget state. The room-fact HMAC uses a separate Worker secret because a plain hash of the short code space would be enumerable; raw codes and the key never enter D1. Production and staging still send ordinary room events to Analytics Engine and D1 best-effort through the Durable Object response header and top-level Worker's `waitUntil`; normal rooms create neither a local outbox schema nor D1 receipts, either sink can miss events, and neither is audit, billing, or user-visible truth. Durable Objects remain authoritative for concurrent room actions and WebSocket broadcasts.
...
#### Schema-v6 receiver, receipt cleanup, and Release-A rollback bridge
...
The receiver has no public route. `cloudflare/worker.ts` now imports it only for the Release-A compatibility consumer: if a Durable Object already contains a valid version-1 local outbox left by a later producer release or rollback, the object's existing alarm can drain the FIFO head one event at a time. Retry count, deterministic backoff, a seven-day/sixteen-attempt bound, and privacy-minimal dead-letter retention are persisted locally. The bridge lazily recognizes existing tables; it does not initialize them for a missing or ordinary room and does not enqueue milestones.

Production and staging remain `ROOM_MILESTONE_DELIVERY_MODE=best-effort`. Normal rooms continue to emit `X-NonStopTalk-Room-Milestones` and use the existing `waitUntil` fan-out, so they create neither local outbox tables nor receipts. Scheduled cleanup deletes expired receipts within the existing bounded schema-6 run and includes remaining receipt work in its shared backlog result; marker-5 cleanup never prepares SQL that names the receipt table. Normal-room production, activation, and end-to-end durable-delivery semantics are not implemented. After a future producer is activated, Release A becomes the minimum safe code rollback because older Workers cannot preserve and drain the local queue. This bridge adds no service, binding, secret, paid product, or D1 migration; the central marker remains 6.
...
- Normal-room production into the implemented version-1 Durable Object SQLite outbox and activation of the internal schema-6 receiver; the current multiplexed FIFO retry/dead-letter consumer is a dormant rollback bridge only

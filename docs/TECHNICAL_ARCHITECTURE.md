...
server-authoritative room milestones
  ├─ current config → best-effort D1 daily rollups + Analytics Engine
  └─ exact outbox → atomic local queue → receipt-gated D1 effects
...
The Cloudflare edition stores each room in its own SQLite-backed Durable Object. State survives hibernation, Worker restarts, and deployments. A per-room alarm deletes storage after 30 days without a state change. Hibernatable WebSockets preserve live connections without holding JavaScript memory active while idle. Under current best-effort production/staging operation, room storage contains only normal room state and no local milestone-outbox tables. Exact Release-B mode can create the version-1 outbox atomically with a milestone mutation, and both Release B and the Release-A rollback bridge recognize its existing tables and multiplex the same alarm with the drain/retry/dead-letter lifecycle.
...
- Operational staging drill and single-version production activation of the implemented exact room-milestone outbox; ordinary deployment propagation is protected by the Release-A-compatible one-header ownership sentinel, while intentional traffic splitting remains outside the rollout plan

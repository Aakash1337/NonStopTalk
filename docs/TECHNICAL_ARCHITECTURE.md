...
- Coaching verification: 36 Node tests for the deterministic engine, practice loop, and storage contract; a Playwright SPA smoke flow asserting no coaching-data API request on the default/off path and exercising IndexedDB compatibility/fallbacks; and separate cloud-progress/platform tests for the allowlist, APIs, ownership, retention, and analytics
...
cloudflare/public/coach-storage.js
                                  IndexedDB v2 persistence and newer-schema
...
IndexedDB access is isolated in `coach-storage.js`. It normally opens schema version 2. If a newer compatible release already upgraded the origin, a `VersionError` makes this release reopen the current version without requesting a downgrade, validate the two required stores, and close its connection on `versionchange`. When an `artifact-lifecycle` store already exists, artifact save, delete, full-clear, and read operations maintain its content-free metadata in the same transaction, honor its recorded expiry times, and conservatively apply the future 128 MiB logical limit. This release does not create that store, so automatic expiry remains inactive on untouched version-2 origins; after a newer schema creates the store, this rollback floor atomically removes expired artifacts and metadata while preserving compact summaries. If browser quota prevents an opted-in artifact save, the failed transaction is retried as summary-only and the UI explicitly reports that the artifact was not retained.
...
JSON export never includes an artifact `Blob` or captured transcript text. Per-attempt download buttons read the local artifact store only. Per-attempt artifact deletion removes that record and resets summary artifact metadata in one transaction while preserving the compact attempt/pair. Confirmed full-history deletion clears every local coaching store and, when enabled and reachable, this anonymous browser's cloud summaries. Untouched version-2 origins have no app-scheduled artifact expiration; if a newer schema has created lifecycle records, the compatibility release honors their 30-day expiry.

The version-2 upgrade preserves an existing version-1 `session-summaries` store and creates `session-artifacts` when missing. The browser smoke suite constructs a v1 database and verifies that both stores exist after Progress opens it; it also exercises the compatibility module against synthetic future schemas, recorded expiry on summary and direct-artifact reads, and real Chromium quota pressure.
...
Local coaching history remains independent of both room persistence systems. It uses best-effort IndexedDB scoped to the current site origin and browser profile. Untouched version-2 origins have no app-scheduled artifact expiration; profiles already upgraded to lifecycle storage keep their recorded 30-day expiry during rollback. The optional D1 backup is a separate compact-summary store controlled by one device-level, UTC-day-bucketed anonymous inactivity lease and a new-save guard once 250 summaries exist. There is no account adoption, recovery, or cross-device sync yet.

...
online browser SPA
  ...
  ├─ per-turn speaker consent + on-device text
  │    └─ capped transcript ───────────────────> same-origin Worker offline judge
  │                                                └─ transcript-free claim/verdict ─> room Durable Object
  ...
  └─ goal + evidence ─> local lexical coaching-card retrieval     │
                         └─ card drill or evidence-safety override  │
                              + deterministic assembly              │
                               └─ IndexedDB v3 ─────────────────────┘
                                  ├─ session-summaries ── optional allowlist ──> Worker /api/v1 → D1
...
server-authoritative room milestones
  └─ production + staging exact outbox → atomic local queue
                                             ├─ receipt-gated D1 effects
                                             └─ best-effort Analytics Engine
external topic attempts ────────> aggregate D1 daily usage budget/counters
```

There are two runtime editions. The richer local/self-hosted game edition is one Go application shared by the normal web and desktop-style launchers. The free online edition is a native TypeScript Worker with a separately tested implementation of the core game rules and offline relevance judge. Its static SPA also contains the browser-only coaching prototype; that prototype is not currently served by the Go application.
...
- AI boundaries: provider-neutral local Go judge/topic-generator interfaces; a pure deterministic Cloudflare offline judge; and separate disabled-by-default Cloudflare topic-provider adapters for direct GLM-4.7 or Workers AI GLM-5.3-Flash routine generation and explicit Gemma 4 31B escalation
- Tests: Go unit/handler/race/vet checks, shared Go/TypeScript game-rule and offline-judge contracts, and Playwright browser smoke flows
...
- Cloudflare verification: Node's test runner through `tsx`, isolated best-effort and exact-outbox Workers runtimes, a false-green-aware Vitest wrapper, browser smokes, recursive exact-source verification for the complete deployable `.js`/`.mjs` Static Assets generation plus negative checks for excluded `*.test.js`/`*.test.mjs` paths, and Wrangler deploy/startup checks
...
testdata/judge-contract.v1.json   versioned cross-runtime offline-judge fixtures
...
cloudflare/judge.ts               pure offline grading, verdict validation, bonus,
                                  and confidence-label contract
...
cloudflare/public/setup-kits.TEST.mjs
                                  setup normalization, corruption, quota, and limits
cloudflare/public/sound-cues.js    dependency-injected browser-local Web Audio cues,
                                  preference migration, and lifecycle cleanup
cloudflare/public/turn-transcription.js
                                  strict on-device, exact-turn transcript lifecycle
...
cloudflare/public/coach-storage.test.js
                                  IndexedDB storage-contract coverage
...
cloudflare/room-milestone-delivery-mode.ts
                                  exact producer gate and public readiness semantics
cloudflare/room-milestone-outbox.ts
                                  private SQLite queue, retry, and dead-letter logic
cloudflare/room-milestone-receiver.ts
                                  receipt-gated D1 milestone application
...
cloudflare/judge-routes.ts        exact-turn judge authorization, transcript-memory
                                  grading, and transcript-free claim resolution
...
`cloudflare/worker.ts` routes public `/api/rooms/{code}` requests to one Durable Object selected by the normalized room code. The object serializes actions, stores the complete game state in its private SQLite database, and broadcasts per-viewer public state through hibernatable WebSockets. `cloudflare/game.ts` owns the online rule implementation; identity tokens and judge transcripts never appear in public state.

Static navigation paths such as `/`, `/practice`, `/progress`, and `/room/ABC234` are served by Workers Static Assets with SPA fallback. The online multiplayer game performs microphone voice-activity analysis locally and commits classic scoring first. Its optional judge is off by default and requires both room-host enablement and fresh consent from the exact speaker for each turn. Strict on-device recognition produces at most 8 KiB of text, which goes once to the same-origin `/api/v1/models/judge` route for the pure deterministic offline heuristic; audio never uploads. The request transcript never enters Durable Object/D1 storage, logs, analytics, history, or a provider, while only transcript-free review coordination and the normalized verdict enter room state. During setup, the separate host-only `/api/v1/models/topics` boundary can produce an editable topic draft from a theme capped at 200 characters. Routine generation is deterministic unless `TOPIC_ROUTINE_PROVIDER=glm` selects direct Z.AI GLM-4.7-Flash or `TOPIC_ROUTINE_PROVIDER=glm53` selects Workers AI GLM-5.3-Flash through the `AI` binding. Gemma 4 31B is available only when `TOPIC_ESCALATION_PROVIDER=gemma31` and the host explicitly selects escalation. Every external path also requires fresh consent for that generation attempt. The separate coaching route can create an explicitly consented transcript only through strict on-device browser recognition; it never uses the topic providers or sends captured transcript text or a recording to the Worker. Compact summary backup is an independent, off-by-default API boundary; captured-transcript/recording retention remains local-only.

The source also imports `cloudflare/public/setup-kits.js` as a separate pure browser module. It stores at most 25 allowlisted, cloned records under a versioned `localStorage` key for the current origin/browser: name (40 Unicode code points), applied duration/silence/rounds, topic-pack ID, and at most 500 custom topics of 200 Unicode code points each. Editor input is capped at 20,000 characters, an imported topic file is capped at 64 KiB before its text is read, and the full serialized store at 512 KiB. Corrupt/future-version data and storage read/quota failures fail closed; a failed write cannot deliberately replace the last valid bytes. This store is unencrypted, best effort, unsynced, and has no recovery.

`cloudflare/public/sound-cues.js` is another pure, dependency-injected browser boundary. It preserves the Go edition's bounded oscillator recipes, constructs one separate cue `AudioContext` lazily after a current-driver gesture, and owns best-effort resume, stale-generation invalidation, node disconnection, and context closure. The default-on preference stores only the exact `off` opt-out under `nonstoptalk.sound`, migrates the legacy key, and synchronizes within the same origin/browser profile. The SPA—not a room snapshot—selects cues from the current local controller, so spectators, passive hydration, reconnects, rejected actions, and stale routes stay silent. Sound duplicates visible timer, voice, and score state and is never the only status signal.

`cloudflare/public/turn-transcription.js` is the isolated browser boundary for a judge turn. It refuses to start without strict `processLocally = true`, the exact live selected audio track, and a current turn identity; it normalizes final text to the 8 KiB request limit and clears it on failure, stale authority, cancellation, or handoff. `cloudflare/judge.ts` is a pure runtime-independent implementation of the same offline formula as Go, including strict verdict validation, fixed feedback, a 0–20 bonus, and confidence labels. The outer Worker claims the exact authorized pending review from the room, grades only in request memory, and resolves the claim with a normalized verdict. The Durable Object owns expiry and exactly-once score application but receives no transcript. Classic scoring is already committed and its baseline remains intact; a review that does not commit adds no bonus, while authoritative room state resolves an ambiguous completion response. Standings/winner remain provisional while a review is pending. The whole slice adds no provider, key, secret, binding, D1 write, migration, Queue, or paid service.
...
The platform slice keeps composition in `cloudflare/worker.ts` and separates live room coordination, relational storage, coaching backup, identity, maintenance health, analytics, milestone delivery, offline judging, and model providers behind small modules. D1 is the central queryable store for anonymous device expiry, consented compact summaries, consent records, HMAC-pseudonymous room facts, one retention-cleanup heartbeat, daily analytics counters, schema-v6 milestone receipts, and aggregate topic-provider usage budget state; judge transcript and verdict fields never enter it (a later ordinary room fact may still contain the room's aggregate total score). The room-fact HMAC uses a separate Worker secret because a plain hash of the short code space would be enumerable; raw codes and the key never enter D1. The committed production and staging policies select exact `outbox`: each deployed healthy lane atomically queues room milestones beside the authoritative room mutation and uses one exact-90-day D1 receipt per delivered milestone to make eligible D1 effects idempotent. Both reported healthy `durable-outbox` during their separate 2026-09-02 activation proofs—staging's rollback/drain/restoration and repeated exact smoke, followed by the production canary's exact seven-receipt, one-room-fact, six-rollup delta. The read-only monitor independently enforces that policy for current deployments. Progress/consent D1 events and Analytics Engine writes remain best-effort in both environments, and none of these sinks is audit, billing, or user-visible truth. Durable Objects remain authoritative for concurrent room actions and WebSocket broadcasts in both modes.
...
#### Schema-v6 receiver, Release-A rollback bridge, and Release-B producer
...
The receiver has no public route. The Release-A compatibility consumer can lazily recognize a valid version-1 local outbox and drain its FIFO head one event per alarm. Retry count, deterministic backoff, a seven-day/sixteen-attempt bound, and privacy-minimal dead-letter retention are persisted locally. A committed retry emits one post-commit structured `room_milestone_outbox_retry_scheduled` record; a lost compare-and-swap emits the distinct non-proof `room_milestone_outbox_retry_stale` record. Release B adds the normal-room producer behind the exact lowercase `ROOM_MILESTONE_DELIVERY_MODE=outbox` gate. On a milestone mutation it commits the room JSON, stable lifecycle metadata, the complete ordered event group, and the multiplexed room/outbox alarm in one Durable Object SQLite transaction before response or broadcast. Expected capacity or canonicalization drops keep gameplay available while incrementing only bounded local counters; other storage or alarm failures roll the room mutation back. Transaction-replay entropy is allocated before the replayable callback so one logical mutation keeps the same lifecycle and event IDs.

Production and staging are explicitly configured exact `outbox`; the read-only monitor requires each current deployment to report healthy `durable-outbox`. In exact mode, the Durable Object uses a comma-only v1 ownership sentinel in the private `X-NonStopTalk-Room-Milestones` header. Release A already splits, trims, filters, and removes this header, so it schedules zero legacy events and cannot leak a newly invented header; Release B recognizes the sentinel as authoritative outbox ownership. An older or intentionally restored best-effort object instead returns real milestone values, preserving the legacy fallback. This one-header response contract makes both directions of normal Worker/Durable-Object version skew safe while preventing dual delivery, and every outer version removes the header from the public response. Status reports `durable-outbox` only when the compiled producer, exact configuration, schema 6, and secure room-fact key are all ready; otherwise an exact configuration reports `degraded-outbox`. Scheduled cleanup deletes expired receipts within the existing bounded schema-6 run and includes remaining receipt work in its shared backlog result; marker-5 cleanup never prepares SQL that names the receipt table. The producer, consumer, and activation add no service, binding, secret, paid product, fixed monthly cost, model, or D1 migration beyond the already-required room-fact key and schema 6. Staging followed the dormant-code-first, single-version cutover and passed rollback/drain/restoration plus repeated exact smoke on 2026-09-02. Production then used a separately reviewed full-version release, target-locked attended quiet-window canary, and verified Release-A-compatible rollback floor before being called proved. Those are historical activation facts, not a claim that the activation Worker remains the current version after later releases. Future high-risk delivery-policy changes require fresh pins and review; routine exact-to-exact releases use the read-only monitor. Once exact mode has produced any local row, Release A is the minimum safe code rollback because older Workers cannot preserve and drain the queue.
...
Finishing writes to IndexedDB database `nonstoptalk-coaching`, version 3. Every completed saved attempt puts an allowlisted record in `session-summaries`: schema version, ID/time, scenario, goal, target duration, explicit practice-loop/baseline IDs plus attempt-role/feedback-mode fields, observed/unknown/coverage measurements, other aggregate measurements, optional transcript counts/patterns, normalized advice, and artifact metadata including `transcriptMayBePartial`. Standalone attempts store null relationship IDs; pre-loop analysis-v2 records that omit the four fields remain legacy standalone attempts. The summary excludes segments, per-frame values, grounding details, audio payloads, and captured transcript text. If full-session retention was selected and a recording or transcript was captured, the same read/write transaction also puts an ID-linked record in `session-artifacts` containing the encoded audio `Blob`, MIME type, captured transcript, partial-text flag, and creation time, plus a required record in `artifact-lifecycle`.

IndexedDB access is isolated in `coach-storage.js`. Schema v3 requires `session-summaries`, `session-artifacts`, and the content-free `artifact-lifecycle` ledger, all keyed by ID; the ledger also has a non-unique `expiresAtMs` index. Each row contains only the ID, `retainedAtMs`, `expiresAtMs`, `logicalBytes`, lifecycle schema version, and migration-grace flag. Logical bytes are the audio `Blob.size` plus the captured transcript's exact UTF-8 byte length. New artifacts use one save timestamp and `expiresAtMs = retainedAtMs + 2_592_000_000`. A v1/v2 upgrade uses one migration timestamp for every structurally valid legacy artifact that needs a backfilled row and gives each the same exact 30-day delta; a valid earlier lifecycle policy is preserved rather than extended.

Before a new artifact is committed, one read/write transaction reconciles summaries, artifacts, and lifecycle rows and calculates aggregate retained bytes. New artifacts share a 128 MiB logical cap. The app never cap-evicts another valid, unexpired artifact; structurally valid migrated artifacts receive `legacyGrace` and remain available for their 30-day migration window even when individually or collectively over the cap. A new artifact that would exceed the cap is deleted or not written, its artifact-presence fields are cleared, and its compact summary commits with `artifactStatus: "app-limit"`. A browser `QuotaExceededError` aborts the combined write before the app retries the same compact summary without an artifact and reports `artifactStatus: "browser-quota"`. If even the summary-only transaction cannot commit, the save fails rather than claiming success.

Storage corruption fails closed. Missing required stores or incompatible key paths reject or atomically abort opening/upgrading rather than reinterpreting content. An old malformed expiry index is repaired inside the atomic v3 upgrade; an incompatible index in an already-current or future schema is rejected. Malformed, future-dated, expired, orphaned, summary-mismatched, or byte-mismatched artifact state is never returned: the artifact and lifecycle row are removed together and only the summary's artifact-presence metadata is reset, preserving its compact measurements and loop relationship. Open connections close on `versionchange`. Both active v3 code and the deployed Release-A compatibility floor recover from `VersionError` by opening the existing compatible version without requesting a downgrade and validating the schema they understand. This makes Release A a safe code rollback after v3 activation, but IndexedDB's version remains 3 or newer and rollback cannot restore content already expired, deleted, or reconciled away.

Future-version compatibility is intentionally additive, not a promise to accept arbitrary schemas. Version 3 freezes the lifecycle-v1 core contract: all three store key paths remain `id`; `artifact-lifecycle.expiresAtMs` remains a non-unique, non-multi-entry index with that exact key path; and the stable row fields retain their v1 meaning and types (`id`, safe `retainedAtMs`, safe `expiresAtMs`, safe `logicalBytes`, lifecycle schema version, and boolean `legacyGrace`). Future code may add fields, stores, or indexes, but must preserve an expiry no later than exactly 30 days after retention and the exact `Blob.size + UTF-8 transcript bytes` accounting understood by Release A and v3. A change to those invariants requires a new compatibility floor rather than being called a compatible future version.
...
`/progress` reads local summaries and, after this browser has opted into cloud backup, merges its reachable D1 summaries by session ID; the local record wins so artifact download metadata remains available. Its local artifact panel comes from one `readCoachingProgressSnapshot()` transaction that reuses the streamed reconciliation, rechecks the clock after scanning payloads, queues a post-normalization summary read, and returns only aggregate numbers plus per-session ID/time/byte policy metadata. It therefore reports exact Blob-plus-UTF-8 logical use, the 128 MiB app limit, migration-grace state, and retention deadlines without loading all recordings together, estimating browser quota, or making a network request. Module-namespace feature detection preserves the Release-B-app/Release-A-storage propagation-skew path by showing an honest unavailable state when the older module lacks the snapshot export. `coach-loop.js` validates explicit relationships and supported setup before grouping. It never pairs by recency: legacy/standalone attempts stay independent, and duplicates, orphans, mismatches, or malformed records remain visible as unpaired. An incomplete valid baseline can restore a locked retry after reload.
...
JSON export never includes an artifact `Blob`, captured transcript text, or lifecycle row. Per-attempt download buttons read the local artifact store only after validating its active ledger row and logical byte count. Per-attempt artifact deletion removes the artifact and required lifecycle row and resets summary artifact metadata in one transaction while preserving the compact attempt/pair. Confirmed full-history deletion clears every local coaching store and, when enabled and reachable, this anonymous browser's cloud summaries. Expired artifacts become unavailable and are removed on the next storage operation after their deadline; the compact summary remains.

The version-3 upgrade preserves an existing version-1 or version-2 `session-summaries` store, creates `session-artifacts` and `artifact-lifecycle` when missing, and backfills valid legacy artifacts atomically. If structural validation or a migration write fails, the upgrade aborts and leaves the prior database version intact. Browser coverage constructs older databases and verifies v1→v3 migration, exact 30-day backfill, UTF-8/Blob byte accounting, app-limit behavior without eviction, expiry on summary and direct-artifact reads, real Chromium quota fallback, Release-A operation on the upgraded origin, restoration to v3 code, version-change closure, and compatible synthetic future schemas.
...
Both editions follow this core sequence; their transcript transport and grading adapters differ.
...
5. Submission includes the intended turn ID, timing result, completion/elimination flags, and that turn's classic/transcript choice. The Go edition can include the consented transcript in this submission; the Cloudflare edition does not.
...
8. If a Cloudflare speaker chose transcription and text was captured, the browser makes one separate same-origin judge request after the room has created that exact pending review. The Worker claims it using the speaker's room token, grades the topic/transcript in memory with the offline heuristic, and sends only the normalized verdict to the room.
9. In the Go edition, an eligible offline or configured provider judge runs asynchronously. Either edition can resolve only its exact pending turn once.

An AI response cannot be applied twice or to a later turn. Go pending judge work cannot resume across a process restart; restore reconciles it to a failed judge state with classic scoring. Cloudflare persists only bounded transcript-free pending metadata in the Durable Object, uses its alarm to expire abandoned reviews, and likewise preserves classic scoring.
...
The Cloudflare game's judge is another separate path. It is default-off at room setup and requires the current speaker's new choice every turn. Strict on-device recognition receives the selected live track; audio stays in the browser, while at most 8 KiB of text goes once to the same-origin Worker. The Worker validates the origin, speaker, turn, transcript shape, and transcript-free room claim, then invokes only the pure offline heuristic. It does not call the topic-provider adapters. The request text is never placed in Durable Object/D1 state, logs, Analytics Engine, Web Analytics, history, or model-usage accounting. Classic scoring lands before that request and its baseline remains intact; a review that does not commit adds no bonus, while authoritative room state resolves an ambiguous completion response. This path adds no paid API, key, secret, binding, provider budget, or migration. A remote GLM/Gemma Cloudflare judge is not enabled and requires a later schema/consent/cost gate.
...
Neither game session model contains transcripts, so they are absent from score history and persisted room snapshots/Durable Object state. The Cloudflare model may contain only transcript-free judge coordination and normalized verdicts. The coaching summary schema likewise excludes captured transcript text; opted-in captured coaching text can exist only in the separate browser artifact store and may carry a partial warning. See [AI and Privacy](AI_AND_PRIVACY.md).
...
The Cloudflare edition stores each room in its own SQLite-backed Durable Object. State can include judge enablement, bounded transcript-free pending-review metadata, and normalized verdicts, but never a transcript. It survives hibernation, Worker restarts, and deployments. A per-room alarm expires abandoned judge reviews and deletes all room storage after 30 days without a state change. Hibernatable WebSockets preserve live connections without holding JavaScript memory active while idle. Production and staging exact Release-B mode can create a bounded version-1 outbox atomically with a milestone mutation, and both Release B and the Release-A rollback bridge recognize its existing tables and multiplex the same alarm with the drain/retry/dead-letter lifecycle.
...
- Bounded concurrent Go judge work, a process-wide Go external-provider budget, and a 30-second Go judge timeout
...
- Same-origin JSON mutations and WebSocket upgrades, 64 KiB bodies, per-source room-creation throttling, a general 60-requests-per-minute `API_RATE_LIMITER` whose `platform` bucket covers both model routes, a five-requests-per-minute `MODEL_RATE_LIMITER` used only by the topic route under its `model-topics` scope, per-member socket caps, and private identity state online
- Exact-speaker/turn authorization, transcript-free bounded claims, one normalized verdict application, a 30-second pending-review expiry, and classic-score fallback for Cloudflare judge work
...
- Setup-kit limits of 25 records, 40 Unicode code points per name, 500 custom topics of 200 code points each, 20,000 editor characters, 64 KiB per imported topic file, and a 512 KiB serialized store; bounded numeric normalization, all-or-nothing topic validation, and one atomic room mutation on Apply
...
- Remote-provider judge parity for Cloudflare; the offline relevance judge is implemented, while any GLM/Gemma judging path remains schema-gated future work

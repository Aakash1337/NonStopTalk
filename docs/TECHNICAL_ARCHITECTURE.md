...
- In-progress web platform: versioned Worker APIs, central D1 repositories/migrations, an internal sync-profile foundation, scheduled anonymous-data expiry, coarse D1/Analytics Engine event aggregation, a dormant schema-v6 milestone-receipt foundation, and aggregate D1 model-usage budgeting
...
#### Schema-v6 milestone receipt foundation
...
The current Worker accepts markers 5 and 6 through an uncached schema guard but executes only schema-v5 application SQL. It never reads or writes the receipt table, so current D1/Analytics Engine milestone fan-out remains fail-open and best-effort. A receipt-gated D1 transaction, Durable Object SQLite outbox, alarm retries, bounded cleanup, and durable-delivery semantics are not implemented by this migration. The bridge remains the safe marker-6 rollback target until those later releases and their rollback window complete.

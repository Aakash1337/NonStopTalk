...
server-authoritative room milestones
  ├─ production best-effort → D1 daily rollups + Analytics Engine
  └─ staging exact outbox → atomic local queue → receipt-gated D1 effects
...
Production remains explicitly configured `best-effort`. Its normal rooms continue to emit real values in `X-NonStopTalk-Room-Milestones` and use the existing `waitUntil` fan-out, so they create neither local outbox tables nor receipts. Staging is explicitly configured exact `outbox` and currently reports healthy `durable-outbox`. In exact mode, the Durable Object uses a comma-only v1 ownership sentinel in that same private header. Release A already splits, trims, filters, and removes this header, so it schedules zero legacy events and cannot leak a newly invented header; Release B recognizes the sentinel as authoritative outbox ownership. An older or best-effort object instead returns real milestone values, preserving the legacy fallback. This one-header response contract makes both directions of normal Worker/Durable-Object version skew safe while preventing dual delivery, and every outer version removes the header from the public response. Status reports `durable-outbox` only when the compiled producer, exact configuration, schema 6, and secure room-fact key are all ready; otherwise an exact configuration reports `degraded-outbox`. Scheduled cleanup deletes expired receipts within the existing bounded schema-6 run and includes remaining receipt work in its shared backlog result; marker-5 cleanup never prepares SQL that names the receipt table. The producer and consumer add no service, binding, secret, paid product, or D1 migration beyond the already-required room-fact key and schema 6. Staging followed the dormant-code-first, single-version cutover and passed rollback/drain/restoration plus repeated exact smoke on 2026-09-02. Production activation must preserve the same sequencing and remains a separate review. Once exact mode has produced any local row, Release A is the minimum safe code rollback because older Workers cannot preserve and drain the queue.
...
The Cloudflare edition stores each room in its own SQLite-backed Durable Object. State survives hibernation, Worker restarts, and deployments. A per-room alarm deletes storage after 30 days without a state change. Hibernatable WebSockets preserve live connections without holding JavaScript memory active while idle. Under current production best-effort operation, room storage contains only normal room state and no local milestone-outbox tables. Staging exact Release-B mode can create the version-1 outbox atomically with a milestone mutation, and both Release B and the Release-A rollback bridge recognize its existing tables and multiplex the same alarm with the drain/retry/dead-letter lifecycle.
...
- Separately review a single-version production activation now that staging rollback/drain/candidate restoration and repeated healthy exact smoke have passed; ordinary deployment propagation is protected by the Release-A-compatible one-header ownership sentinel, while intentional traffic splitting remains outside the rollout plan

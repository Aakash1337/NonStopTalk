...
Schema v6 is an additive receipt foundation. `room_milestone_receipts` started empty and permits only opaque lowercase 256-bit event IDs/payload hashes plus canonical UTC receipt, optional application, and exact 90-day expiry timestamps. The Worker includes a strict internal schema-6 receiver that receipt-gates one canonical D1 application, plus bounded schema-6 expiry cleanup. Marker-5 cleanup never prepares receipt-table SQL. Release A can lazily recognize and drain a version-1 local Durable Object outbox. Release B adds the matching producer: exact `outbox` mode atomically commits room state, stable lifecycle metadata, the complete ordered event group, and the multiplexed alarm before responding or broadcasting. Expected capacity/canonicalization drops commit gameplay and bounded counters without a legacy fallback; storage or alarm failures roll the whole mutation back. The committed production and staging policies select exact `outbox`; each delivered milestone on a healthy `durable-outbox` deployment creates one receipt with exact 90-day expiry so eligible D1 effects are replay-idempotent. Staging passed its rollback/drain/restoration proof plus repeated exact smoke on 2026-09-02, and production then passed its separate attended quiet-window activation canary with the exact seven-receipt, one-room-fact, six-rollup delta. Release A is the minimum safe rollback floor after any rows have been produced.
...
| `room_milestone_receipts` | One exact-90-day idempotency receipt per delivered production or staging room milestone; bounded cleanup expires them | Room codes, names, topics, member/authentication tokens, IP data, audio, transcripts, coaching content |
| `analytics_daily` | Queryable daily event rollups; production/staging room-milestone increments are receipt-gated and progress/consent increments remain best-effort | Per-person identifiers |
...
Analytics Engine receives the same coarse event vocabulary for inexpensive time-series exploration. D1 rollups provide a small protected readout for the admin API. Production and staging exact outbox use the internal receiver to receipt-gate each eligible room-milestone D1 batch and make replay idempotent. Progress/consent D1 events remain best-effort through `waitUntil` in both environments. A newly applied room milestone then offers Analytics Engine one best-effort post-commit opportunity; that opportunity is not retried after a failure or interruption. Analytics Engine is therefore never delivery-exact, and neither analytics surface is an audit or billing ledger.
...
- Add server-authoritative room lifecycle events, room facts, and daily counters; production and staging room-milestone D1 increments are receipt-gated, while progress/consent D1 increments and every Analytics Engine write remain best-effort.
...
- **Activated and proved in production:** exact `outbox` provides durable, replay-idempotent eligible room-milestone D1 effects when readiness is healthy. Staging passed rollback-to-Release-A drain, independent legacy control, candidate restoration, and repeated exact smoke on 2026-09-02; the separate full-version production release, target-locked canary, and verified Release-A-compatible rollback floor then passed the same day. Routine exact-to-exact releases use the read-only health workflow rather than repeating that mutating proof.
...
- Do not write D1 on socket presence ticks or page views. Persist summaries and consent changes, coarse room-milestone/analytics aggregates, and aggregate provider-budget counters. Production and staging exact outbox write one bounded local event per milestone and one exact-90-day D1 receipt per delivered event, without adding a service, binding, secret, migration, model call, Queue, another database, paid product, or fixed monthly cost.

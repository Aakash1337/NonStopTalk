...
- **Configured for production after staging proof:** exact `outbox` provides durable, replay-idempotent eligible room-milestone D1 effects when readiness is healthy. Staging passed rollback-to-Release-A drain, independent legacy control, candidate restoration, and repeated exact smoke on 2026-09-02. Production activation remains an attended post-merge operation and is not complete until the separate full-version review, target-locked canary, and verified immediate rollback version all pass.

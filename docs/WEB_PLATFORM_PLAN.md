...
Browser SPA
  ...
  |-- versioned HTTPS API
          ...
          |                         dormant v1-outbox rollback consumer)
...
Schema v6 is an additive receipt foundation. `room_milestone_receipts` starts empty and permits only opaque lowercase 256-bit event IDs/payload hashes plus canonical UTC receipt, optional application, and exact 90-day expiry timestamps. The Worker includes a strict internal schema-6 receiver that can receipt-gate one canonical D1 application, plus bounded schema-6 expiry cleanup. Marker-5 cleanup never prepares receipt-table SQL. Release A now imports that receiver and can lazily recognize and drain an already-existing version-1 local Durable Object outbox from a later producer release or rollback. The dormant consumer processes only the FIFO head, one event per alarm, and persists bounded retry/dead-letter state. Production and staging remain `best-effort`: normal rooms use the response-header plus `waitUntil` path, never initialize the local outbox schema, and do not insert receipts. Normal-room production, activation, and any end-to-end durable-delivery claim remain future work. After production is eventually activated, Release A becomes the minimum safe rollback floor because older Workers cannot drain the rows.
...
| `room_milestone_receipts` | Idempotency receipts for the internal schema-6 receiver; normally empty under current best-effort traffic, but writable by an explicit test/internal call or a pre-existing local-outbox drain, and subject to bounded expiry cleanup | Room codes, names, topics, member/authentication tokens, IP data, audio, transcripts, coaching content |
...
Analytics Engine receives the same coarse event vocabulary for inexpensive time-series exploration. D1 rollups provide a small protected readout for the admin API, but the normal room paths are best-effort: D1 writes fail open, room-triggered writes run through `waitUntil`, and events can be missed. The internal receiver receipt-gates its D1 batch when explicitly invoked or reached by a pre-existing local-outbox drain, then offers Analytics Engine one best-effort post-commit opportunity only for a newly applied event; that opportunity is not retried after a failure or interruption. Neither store is an audit log, billing ledger, or delivery-exact source of truth.
...
- **Internal foundation implemented:** add the constrained schema-v6 milestone-receipt table, strict canonical receiver, bounded receipt cleanup, and a Release-A compatibility consumer that can drain a pre-existing version-1 Durable Object outbox without activating normal room delivery.
- **Still future:** produce normal-room milestones atomically into that local outbox and activate receipt-gated delivery. The FIFO one-at-a-time alarm consumer, persisted retry limits, and privacy-minimal dead-letter lifecycle already exist but remain dormant for ordinary traffic.
...
- Do not write D1 on socket presence ticks or page views. Persist summaries and consent changes, attempt coarse room-milestone/analytics aggregates, and retain aggregate provider-budget counters. Normal room traffic creates neither a local outbox nor receipts; only explicit internal receiver/test calls or a pre-existing version-1 local-outbox drain can insert receipts, and schema-6 cleanup can delete expired rows.

...
| `room_milestone_receipts` | Idempotency receipts for the internal schema-6 receiver; normally empty under current production/staging best-effort traffic, populated by exact outbox delivery or an explicit internal/test call, and subject to bounded expiry cleanup | Room codes, names, topics, member/authentication tokens, IP data, audio, transcripts, coaching content |
...
Analytics Engine receives the same coarse event vocabulary for inexpensive time-series exploration. D1 rollups provide a small protected readout for the admin API. Current production/staging rooms remain best-effort through `waitUntil`; in exact outbox mode, the internal receiver receipt-gates the eligible D1 batch and makes replay idempotent. It then offers Analytics Engine one best-effort post-commit opportunity only for a newly applied event; that opportunity is not retried after a failure or interruption. Analytics Engine is therefore never delivery-exact, and neither analytics surface is an audit or billing ledger.
...
- **Still future:** complete the dormant staging/rollback drill and make the separate production `outbox` configuration cutover. Committed production and staging remain `best-effort` until that promotion gate passes.

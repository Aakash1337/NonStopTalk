...
6. **Separate artifact choice.** An unchecked `MediaRecorder` option can retain the active-attempt recording and any available captured transcript in a second local store. Recognition gets up to two seconds to finish; if it times out or errors after returning text, Review and Progress warn that the retained transcript may be partial. IndexedDB v3 atomically pairs each artifact with a required content-free lifecycle row. New artifacts get exactly 30 days, v1/v2 artifacts get exactly 30 days from migration, and a 128 MiB logical cap blocks only the new artifact instead of evicting valid retained content. Progress reports exact local logical use and absolute retention deadlines without claiming to know the browser's separate quota. These artifacts never upload and can be deleted per attempt without deleting their compact summary or comparison.
...
| Retention-boundary violations | Automated checks that full retention starts unchecked, artifacts appear only after that choice, summary export excludes full artifacts, per-attempt deletion preserves the compact pair, and full-history deletion clears all local coaching stores. | Stop release and repair consent/storage boundaries before adding more data features. |
...
The separate unchecked full-session-retention option records the active attempt. It keeps a captured transcript only when transcript analysis also returned text. Recordings and captured transcripts live in a separate origin-local artifact store, never enter cloud backup, and are excluded from JSON export. A finalization error/timeout is visibly marked possibly partial. IndexedDB v3 keeps a separate content-free ledger and gives new artifacts exactly 30 days; migrated artifacts get the same exact window from upgrade. The 128 MiB logical cap never evicts another valid artifact, and either an app-limit or browser-quota outcome retains only the compact summary with an explicit warning. Progress can delete one attempt's artifacts while preserving its summary/pair. Browser storage remains best-effort and may disappear earlier; cleanup after the deadline happens on a later storage access. The Release-A rollback floor can use an already-upgraded compatible database, but cannot restore an artifact already deleted or expired. Deleting browser data cannot delete downloaded files.
...
Thirty-four deterministic coaching/loop tests plus eight IndexedDB storage-contract tests (42 total) cover calibration, segmentation, confidence, tips, transcript analysis, retrieval, grounding safety, template assembly, relationship validation, persistence gating, safe grouping, goal-specific comparison, advice, and the v3 lifecycle/snapshot contract. Browser coverage exercises both feedback modes, baseline → Progress/reload/resume → retry, local storage, exact artifact usage/deadline display, v1→v3 migration, exact expiry, artifact-only deletion, Release-A/future-schema compatibility, fail-closed malformed storage, 128 MiB app-limit and real browser-quota summary-only outcomes, and no coaching-data API request on local-first default/off paths. Separate cloud-progress and platform tests cover relationship metadata, legacy compatibility, the summary allowlist, reserved D1 columns, versioned APIs, anonymous ownership/expiry, and aggregate analytics. These are implementation checks, not real-device accuracy, accessibility, privacy certification, fairness, usefulness, or learning-outcome studies.

The lifecycle-specific browser evidence comes from `smoke:coach-storage`, which swaps a hash-pinned Release-A fixture and current code on the same origin; `smoke:coach` remains the end-user Practice/Progress integration flow.

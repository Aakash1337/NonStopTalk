...
6. **Separate artifact choice.** An unchecked `MediaRecorder` option can retain the active-attempt recording and any available captured transcript in a second local store. Recognition gets up to two seconds to finish; if it times out or errors after returning text, Review and Progress warn that the retained transcript may be partial. IndexedDB v3 atomically pairs each artifact with a required content-free lifecycle row. New artifacts get exactly 30 days, v1/v2 artifacts get exactly 30 days from migration, and a 128 MiB logical cap blocks only the new artifact instead of evicting valid retained content. These artifacts never upload and can be deleted per attempt without deleting their compact summary or comparison.
...
Thirty-four deterministic coaching/loop tests plus seven IndexedDB storage-contract tests (41 total) cover calibration, segmentation, confidence, tips, transcript analysis, retrieval, grounding safety, template assembly, relationship validation, persistence gating, safe grouping, goal-specific comparison, advice, and the v3 lifecycle contract. Browser coverage exercises both feedback modes, baseline → Progress/reload/resume → retry, local storage, v1→v3 migration, exact expiry, artifact-only deletion, Release-A/future-schema compatibility, fail-closed malformed storage, 128 MiB app-limit and real browser-quota summary-only outcomes, and no coaching-data API request on local-first default/off paths. Separate cloud-progress and platform tests cover relationship metadata, legacy compatibility, the summary allowlist, reserved D1 columns, versioned APIs, anonymous ownership/expiry, and aggregate analytics. These are implementation checks, not real-device accuracy, accessibility, privacy certification, fairness, usefulness, or learning-outcome studies.

...
The schema-5/6 bridge was deployed and verified on marker 5 before `0006_room_milestone_receipts.sql` was added. Migration 0006 is an additive, schema-only step: it creates an empty `room_milestone_receipts` table and advances the marker to 6 without changing any schema-v5 table, column, constraint, query, or behavior. The bridge accepts and reports only markers 5 and 6, performs an uncached singleton marker read before every logical D1 operation, and continues to execute only schema-v5 application SQL. Unsupported markers therefore fail closed immediately without a Worker restart, and this bridge remains the designated code-rollback target after the database reaches marker 6.

The new table is not read or written by this release. It can contain only opaque lowercase 256-bit event IDs and payload hashes plus canonical UTC receipt, optional application, and exact 90-day expiry timestamps. It has no room code, name, topic, member/authentication token, IP data, audio, transcript, or coaching content. No receipt receiver, Durable Object outbox, retry loop, cleanup write, or durable/exact-delivery guarantee is active yet; room milestones retain the existing best-effort `waitUntil` path. Keep production and staging probes compatible with markers 5 and 6 through the receiver/outbox rollout and rollback window, then contract the feature Worker and probes to exact marker 6. Do not contract the device-owned schema until later compatibility releases and retention/deletion validation are complete.

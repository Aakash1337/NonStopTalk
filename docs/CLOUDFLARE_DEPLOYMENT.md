...
The production exact-outbox canary is intentionally separate because it creates one synthetic room lifecycle and checks exact aggregate-only production D1 deltas, including a delayed confirmation. Run it attended in a quiet window, only after the reviewed version is fully deployed and the read-only probe is healthy. It is target-locked to `https://dontstoptalking.org` and the production `PLATFORM_DB`; its sole argument is the independently verified immediate code-rollback version. For this activation release, that reviewed rollback version is `58df8c9f-b4d7-4f3e-b15c-32dfec579355`:

```sh
npm run smoke:production-outbox -- 58df8c9f-b4d7-4f3e-b15c-32dfec579355
```

That UUID is a release-specific rollback bookmark, not permanent configuration and not a D1 Time Travel bookmark. Record and pass a freshly reviewed rollback version for a later release. This is a quiet-window aggregate proof, not cryptographic per-event attribution: a permanently missing canary event plus an unrelated identical event could theoretically substitute, so discard an overlapped or ambiguous run.
...
Worker to report `durable-outbox`. It does not install dependencies or receive Cloudflare

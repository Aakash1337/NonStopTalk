...
Setup kits store their name, currently applied duration/silence/rounds, topic-pack choice, and custom topics unencrypted in best-effort `localStorage` for that origin/browser profile. They are unsynced and have no server recovery. The bounds are 25 kits, 40 Unicode code points per name, 500 custom topics of 200 code points each, 20,000 editor characters, and 512 KiB for the serialized store. Import fills only the editor draft until explicit use; an exported `nonstoptalk-topics.txt` file is outside app deletion/control. Deleting coaching history does not delete this separate game store.

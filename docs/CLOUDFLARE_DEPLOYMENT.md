...
- a Release-A-compatible FIFO consumer plus a configuration-gated Release-B producer, enabled in production and staging, that atomically queues room milestones in version-1 local Durable Object outboxes without adding another Cloudflare service
- a default-off, per-turn-consented on-device transcription module and same-origin Worker offline relevance judge
...
The multiplayer game uses the Worker and Durable Objects. Speech/audio analysis and recordings remain in the browser, and microphone audio is never uploaded. The optional game judge is off by default; after the exact speaker consents for a turn, strict on-device recognition sends only that capped transcript once to the same-origin Worker for memory-only offline grading. Transcript text never enters Durable Object/D1 state, logs, analytics, history, or a provider. Compact-summary backup is a separate, off-by-default coaching choice: when selected, the SPA sends only allowlisted measurements/advice to the versioned Worker API and D1. With backup off, coaching makes no coaching-data API request. No coaching data enters a room Durable Object or external model. Separately, a room host can consent to one topic-generation attempt; the normalized theme, capped at 200 characters, is the only host or room content sent to the provider. The request never includes coaching data, audio, transcript text, names, or a room token.
...
This small party game is designed to stay inside those allocations: inactive objects do not accrue duration, and the WebSocket Hibernation API lets an idle room sleep without disconnecting its players. The platform also avoids D1 writes for page views and presence ticks, uses one day-bucketed retention lease per anonymous browser, rejects new saves once 250 summaries exist, and uses small daily aggregate rows. Setup-kit save/delete and topic-text import/export are local, uninstrumented operations and consume none of those Cloudflare allocations; Apply uses the existing room request and persistence only, with no D1, Analytics Engine, or model-provider work. The offline judge adds one bounded same-origin Worker request only after speaker consent. A successful review also performs two internal Durable Object fetches and two room-state saves with alarm reconciliation for claim and resolution; it still adds no D1, Analytics Engine, provider, model, secret, binding, migration, or fixed monthly cost. Progress/consent D1 analytics and every Analytics Engine write remain best-effort in both environments; the committed production and staging exact-outbox policy makes only eligible room-milestone D1 effects durable/idempotent after its readiness gates pass. Schema v4 adds one small profile row and one membership row per browser, plus bounded cleanup. The outbox activation adds no service, binding, secret, migration, model call, paid product, or fixed monthly cost. It reuses each room's bounded private SQLite outbox and existing alarm and adds one D1 receipt, retained for exactly 90 days, per delivered milestone. The staging rollback/drain/restoration proof and separate production quiet-window activation canary both passed on 2026-09-02. Workers Free does not silently begin paid overage billing. If a daily allocation is exhausted, affected operations may fail until the allocation resets.
...
The Worker also uses Cloudflare's built-in [Rate Limiting binding][rate-limit-binding] to allow at most ten room creations per source connection per minute in each Cloudflare location. A general `API_RATE_LIMITER` allows 60 requests per source connection per minute in each scoped platform, room-read, room-connect, or room-mutation bucket, falling back to the anonymous browser identity in local development. Both `/api/v1/models/topics` and `/api/v1/models/judge` traverse its `platform` bucket. Only the external-capable topic route then traverses the dedicated `MODEL_RATE_LIMITER`, capped at five requests per source connection per minute under the `model-topics` scope; the deterministic offline judge does not consume that provider limiter. Limiter keys are SHA-256 digests rather than raw addresses or browser tokens. These permissive, eventually consistent counters reduce trivial Free-plan quota abuse; they are not authentication, accounting boundaries, or a defense against a distributed attack.

The supplied Cloudflare multiplayer game now has an offline-only optional judge. Its room setting is off by default, each speaker consents again for their own turn, and classic scoring lands before the judge request and remains the fallback. It is free and deterministic and does not use the configured topic-provider tiers. The separate host-only theme generator is also deterministic by default. Operators may use direct Z.AI GLM-4.7-Flash as the strict-free routine topic option, Workers AI GLM-5.3-Flash as the preferred cheap Workers Paid routine topic option, and independently make Gemma 4 31B available for explicit host topic escalation. Remote GLM/Gemma judging remains later, schema-gated work. Cloud coaching continues to use bundled cards, local lexical retrieval, deterministic templates, and optional strict on-device recognition; it never uses any topic provider. External coaching AI, Queues, and R2 remain future modules, not deployment requirements.
...
## First-time provisioning and deploy
...
Do not rerun `npm run db:create`, replace an environment's committed D1 database ID, or recreate/reset its existing secrets during a routine release. Those are provisioning operations, not update commands; use `wrangler secret put` later only for an intentional secret addition or rotation. Run the verification commands through `npm run smoke:multiplayer` above, promote and verify the isolated environment first with `npm run deploy:staging`, then use the normal `npm run deploy` production path. Each deployment script applies only pending append-only migrations before deploying code. Finish with the read-only `npm run smoke:production` probe and confirm the scheduled two-environment health workflow remains green.
...
The one-time production exact-outbox activation canary completed on 2026-09-02. It created one synthetic room lifecycle and observed the exact seven-receipt, one-room-fact, six-rollup aggregate delta with delayed confirmation. Do not rerun that mutating command for routine releases or monitoring. Routine deployment verification is the read-only `npm run smoke:production` probe plus the scheduled/manual `Production health` workflow. A future high-risk delivery-policy change must use the separately reviewed, freshly pinned procedure in [Production operations](PRODUCTION_OPERATIONS.md), not the completed activation command or its historical rollback bookmark. The canary evidence is aggregate rather than cryptographic per-event attribution.
...
`.js`/`.mjs` Static Assets manifest under `cloudflare/public` (currently 12
deployed files). It requires canonical UTF-8 without a BOM and exact checked-out source,
...
core SPA import/consumption/export checks remain in place. The source contract
uses `.assetsignore` to exclude exact case-insensitive `*.test.js` and
`*.test.mjs` paths from both deployments; the current inventory contains nine
excluded test modules, and each live row negatively verifies every discovered
excluded path resolves to protected HTML rather than executable JavaScript.
It does not install dependencies or receive Cloudflare credentials, so it
cannot deploy, migrate, or write either environment. A
...
The committed production and staging environments intentionally select exact `outbox`; the read-only monitor requires each current deployment to report schema-6 `durable-outbox` readiness. Each promotion preserves its environment's mode. Reserve a quiet staging window outside cleanup and UTC rollover, then run `npm run smoke:staging-outbox`. That command hard-refuses every non-staging origin, requires healthy schema-6 `durable-outbox` readiness before mutation, drives one synthetic two-player lifecycle, rejects leaked private protocol headers, and verifies the exact seven-receipt, one-room-fact, and six-rollup-event deltas through a fixed aggregate-only remote D1 query. Unexpected aggregate movement fails the check, but the privacy-minimal store has no probe correlation ID: a permanently missing synthetic event plus an unrelated identical event could theoretically substitute. That limitation makes a quiet window mandatory. Discard any overlapped or ambiguous run and restart from a fresh quiet-window baseline rather than weakening the assertion. The rollback/drain/candidate-restoration proof, repeated exact staging smoke, and separate production activation proof reported healthy readiness and passed on 2026-09-02. Neither smoke changes the configured mode; do not substitute the production activation canary for routine read-only health checks.
...
Open `http://127.0.0.1:8787/practice` for coaching or `/progress` for local history and optional cloud summaries. A different port is a different IndexedDB, `localStorage`, and anonymous-cookie origin. These pages do not create a Durable Object. Coaching microphone processing, optional recordings, captured transcripts, and saved game setup kits stay in the browser; only explicitly backed-up compact summaries enter local/remote D1. On `/room/...`, the separate optional game judge can send a consenting speaker's capped on-device transcript to that same local Worker process for request-memory-only grading; it still never enters D1 or Durable Object storage.
...
## Optional offline game judge

The judge is disabled in every new room. A host may offer it in setup, but only the exact speaker can choose on-device transcription for their current turn; that consent is requested again next turn, and starting the manual timer chooses classic play. The browser requires `SpeechRecognition.processLocally === true` and supplies the same selected live microphone track used for turn detection. If capability, track, capture, or final text is unavailable, the turn continues and scores classically without a judge request.

For a consented turn, the browser sends one same-origin `POST /api/v1/models/judge` after the turn result creates an authorized pending review. The request contains the room code, exact turn ID, `externalConsent: false`, and a normalized nonempty transcript capped at 8 KiB. It contains no audio, name, topic, room snapshot, or provider selector. The Worker validates exact origin and shape, uses the room token to claim that speaker's exact pending turn, receives the assigned topic from the Durable Object, and grades topic/transcript synchronously with the deterministic offline heuristic. The browser clears its working transcript immediately after initiating the request.

The transcript exists only in browser memory and the Worker's request memory. It is never stored in Durable Object SQLite, D1, room/public state, history, logs, traces, Analytics Engine, model-usage rows, or Web Analytics, and it is never sent to a topic/model provider. The Durable Object stores only bounded transcript-free review metadata while pending and the normalized relevance/confidence/feedback verdict afterward. Classic scoring is committed first and its baseline is never removed; the verdict can add `round(relevance × 20)` points, capped at 20, to that exact turn once. Standings and the winner remain provisional while a review is pending. A stale/duplicate claim, invalid verdict, or review that does not commit before its deadline expires or resolves fail-closed with no bonus. If a completion response is lost after commit, authoritative room state confirms whether that one-time bonus was applied.

This J1 release introduces no D1 migration, row, API key, Wrangler secret, model-usage budget reservation, new binding, Queue, paid API, or fixed monthly cost. Deploy it with the existing Worker/Assets command and existing platform prerequisites; there is no judge-specific provisioning step. The current GLM-4.7 Flash, GLM-5.3 Flash, and Gemma 4 31B adapters below remain topic-only. Using one for a future remote judge requires an explicit schema/version gate, separate external-processing consent and privacy copy, provider/cost controls, and rollback tests; it is not enabled by changing the current topic variables.

...
The schema-5/6 bridge is a compatibility-only Worker release and includes no migration. It accepts and reports only markers 5 and 6 while continuing to read and write only the schema-v5 SQL contract. Every logical D1 operation performs an uncached singleton marker read first, so unsupported markers fail closed immediately without a Worker restart. Deploy and verify this bridge against marker 5 before adding or applying migration `0006`. Migration 0006 must be additive and preserve every schema-v5 table, column, constraint, and behavior required by this Worker, which keeps the bridge rollback-safe after the database reaches marker 6. Validate the bridge on both markers and exercise the old/new Worker paths before promotion. Once the schema-v6 feature Worker is deployed and its rollback window closes, narrow that Worker and the production and staging probes to require exactly marker 6. Do not contract the device-owned schema until later compatibility releases and retention/deletion validation are complete.

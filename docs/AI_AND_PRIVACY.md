...
- The central D1 platform store holds only the allowlisted records described above: anonymous device ownership/expiry, consented compact summaries, consent records, 90-day HMAC-pseudonymous room facts, retained best-effort daily analytics aggregates, aggregate daily model-usage budget counters, and schema-v6 receipts used only by explicit internal receiver/test calls. Normal room traffic leaves the receipt table empty, and scheduled schema-6 cleanup removes expired rows. Receipt fields cannot contain room codes, names, topics, member/authentication tokens, IP data, audio, transcripts, or coaching content. Themes, generated topic text, raw room codes, and the HMAC key are not stored in D1.

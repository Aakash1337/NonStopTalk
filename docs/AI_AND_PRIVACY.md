...
Release A also contains a dormant local compatibility consumer. If a later producer release or rollback has already left a valid version-1 outbox in a room's private Durable Object SQLite storage, the Worker can drain its FIFO head one event per alarm through persisted bounded retry and privacy-minimal dead-letter state. The canonical queued payload excludes the raw room code, names, topics, member/authentication tokens, IP data, audio, transcripts, and coaching content; terminal dead letters retain only bounded reason/milestone/attempt/timing metadata, not the event ID or payload. Production and staging remain `best-effort`, so ordinary rooms never create those local tables, enqueue events, or insert receipts. Normal-room durable production and an end-to-end delivery guarantee remain unimplemented. The Progress delete control clears local stores and, when cloud backup is enabled and reachable, the current anonymous browser's D1 summaries. There is still no account, recovery flow, app-level encryption, or cross-device synchronization.

Product analytics are coarse aggregates, not a copy of coaching progress. Server-authoritative room milestones and accepted summary-save/delete/consent transitions attempt small daily D1 increments and Analytics Engine writes. The ordinary runtime sends both sinks best-effort: failures are caught so rooms and coaching continue, writes use `waitUntil`, and events can be missed. The internal receiver has a narrower idempotency boundary for explicit canonical deliveries or a pre-existing local-outbox drain: it receipt-gates the D1 changes, then offers Analytics Engine at most one best-effort post-commit write for a newly applied event. A failure or interruption in that final step can still lose the Analytics Engine event, and replaying the receipt does not retry it. Release A's public status always reports delivery as `best-effort`, even after an accidental mode edit; only a future producer release may report `durable-outbox` after exact `outbox` mode plus schema-6/readiness gates. Aggregate event values may include turn/session duration or completed-turn counts. Separate aggregate D1 model-usage rows enforce the external-topic daily budget; they contain no theme or generated topic text. Analytics and usage counters exclude names, IP addresses, user agents, raw browser tokens, room-member tokens, audio, captured transcripts, word patterns, advice, and delivery-quality measurements such as speaking ratio. The protected admin API queries D1 rollups, but those rows are operational estimates rather than audit, billing, or delivery-exact truth.
...
- The central D1 platform store holds only the allowlisted records described above: anonymous device ownership/expiry, consented compact summaries, consent records, 90-day HMAC-pseudonymous room facts, retained best-effort daily analytics aggregates, aggregate daily model-usage budget counters, and schema-v6 receipts used by explicit internal/test calls or a pre-existing local-outbox drain. Normal room traffic leaves the receipt table empty, and scheduled schema-6 cleanup removes expired rows. Receipt fields cannot contain room codes, names, topics, member/authentication tokens, IP data, audio, transcripts, or coaching content. Themes, generated topic text, raw room codes, and the HMAC key are not stored in D1.

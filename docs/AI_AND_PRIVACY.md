...
IndexedDB is scoped to a site origin (scheme, host, and port) within a browser profile. Records are best-effort browser storage: site-data deletion, private browsing behavior, or storage pressure may remove them. On untouched version-2 origins, local artifacts have no app-scheduled expiration and may remain until the user deletes that attempt's artifacts, all local history, or site data. If a newer schema has already created lifecycle records, the compatibility release continues honoring their 30-day expiry during rollback. A custom domain, `workers.dev`, `127.0.0.1:8787`, and another local port each have separate local history.

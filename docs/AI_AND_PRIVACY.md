...
This document describes the current NonStopTalk implementation and the explicitly marked in-progress platform slice. There are five distinct paths with different boundaries:

1. The local and online multiplayer game, which can run with classic scoring and no transcript.
2. The optional, disabled-by-default offline relevance judge in the Cloudflare game edition.
3. The optional, disabled-by-default theme-to-topics providers in the Cloudflare game edition.
4. The optional AI judge and topic generator in the local Go game edition.
5. The speech-coaching prototype in the native Cloudflare SPA, which performs analysis and curated coaching-card retrieval in the browser, with an off-by-default API for compact-summary backup and no external coaching/AI service.

NonStopTalk does not upload microphone audio in any of these paths. In the Cloudflare game judge, the consenting speaker's browser creates text on-device and sends only that capped current-turn transcript to the same-origin Worker for memory-only offline grading. In coaching only, a separate unchecked option can record the active attempt and retain it locally in this site's browser storage. That coaching recording and any captured coaching transcript are never sent to the Worker or another service. A different explicit choice may send only an allowlisted compact measurement/advice summary to NonStopTalk's D1 database.
...
| Cloudflare judge disabled, or speaker chooses classic/manual | Read locally by Web Audio when used; never uploaded | None | None |
| Cloudflare offline judge enabled and exact speaker consents for this turn | The official client reads it locally with Web Audio and on-device recognition and does not upload it | Up to 8 KiB goes once to the same-origin Worker under a 15-second client-memory deadline, exists only in browser/Worker request memory, and never enters room state, D1, logs, analytics, history, or a provider | None; the Worker's deterministic heuristic grades it |
...
## Cloudflare offline game judge

The online judge is off in every new room. The host can make it available, but that is not consent on a player's behalf. Before each exact turn, that speaker chooses either classic play or on-device transcription; the choice is not carried forward, and the manual timer always selects classic. Transcription starts only when the browser exposes `SpeechRecognition.processLocally`, preserves `processLocally = true`, and accepts the same live microphone track used for the turn. If any condition fails, play continues with classic scoring and no transcript request.

After a consented microphone turn, the official client normalizes and caps the text at 8 KiB. As soon as capture finishes, it moves that text under one 15-second in-memory delivery deadline; the same abort signal covers the prerequisite classic-score submission and the single request carrying room code, exact turn ID, transcript, and `externalConsent: false` to `/api/v1/models/judge`. The recognition working copy is cleared immediately, and timeout, room navigation, or page teardown aborts the remaining delivery lease. This official flow does not upload microphone audio. The Worker validates same-origin access, that exact completed-turn speaker's authorization, the pending turn identity, and the transcript shape, then grades the assigned topic and transcript synchronously with the shared deterministic heuristic. Each internal room claim/resolve call is abortable and logically capped at five seconds, with resolve additionally capped by the exact remaining review window. The Worker neither calls a model/provider nor retains the text after the request.

Only transcript-free review coordination crosses into the room Durable Object: exact turn/player/tier/deadline metadata, an opaque claim ID while work is pending, and the normalized verdict when it resolves. Transcript text is never a Durable Object field and never reaches its SQLite storage, room/public state, game history, D1, Analytics Engine, application logs, Cloudflare model-usage counters, topic-provider adapters, or Cloudflare Web Analytics. The resulting verdict contains only bounded relevance/confidence values and fixed safe feedback; the bonus is `round(relevance × 20)`, capped at 20.

Classic scoring is committed first and its baseline is never removed. A completed offline review may add its bonus to that exact turn once; standings and a winner are labeled provisional while any review remains pending. Missing/empty text, stale authorization, unsupported recognition, a rejected or duplicate claim, an invalid verdict, or a review that does not commit before its deadline adds no bonus. If a completion response is lost after the Durable Object commits, the authoritative room state confirms whether that one-time bonus was applied. The room's bounded pending metadata can survive hibernation, but it expires fail-closed and cannot make transcript text durable.

This is a casual party-game trust boundary, not transcript attestation or anti-cheat. The Worker proves only that the browser token belongs to the exact speaker and pending turn. Because the official flow deliberately uploads no audio, the Worker cannot prove that a modified client's submitted text came from the live microphone; such a client can fabricate keyword-rich text. The host's existing score correction is the recovery control.

This J1 path adds no paid API, provider attempt, model-usage reservation, key, Wrangler secret, binding, Queue, D1 row, or migration. Direct GLM-4.7 Flash, Workers AI GLM-5.3 Flash, and Gemma 4 31B remain available only to the separate topic-generation boundary described below. A future remote Cloudflare judge may reuse a provider adapter only after a new schema/version gate, explicit external-processing consent, cost controls, provider-specific retention disclosure, and fail-closed rollout tests are designed; it is not an automatic escalation or fallback from the offline judge.
...
## Cloudflare setup-kit storage and application

The Cloudflare source lets the current room host save the currently applied setup as a named kit: speaking duration, silence limit, rounds, topic-pack ID, and custom topics. The dependency-free store keeps only allowlisted fields and cloned values under the versioned key `nonstoptalk.setup-kits.v1`. It allows at most 25 kits, limits names to 40 Unicode code points, limits custom lists to 500 topics of 200 Unicode code points each, limits editor text to 20,000 characters, rejects an imported file over 64 KiB before materializing its text, and limits the complete serialized store to 512 KiB. Duplicate-name replacement requires an explicit overwrite.

The store is unencrypted, origin/browser-profile-scoped `localStorage`. It is best effort: site-data clearing, private browsing behavior, browser policy, storage pressure/quota, corruption, an unsupported future format, or a read/write failure can remove kits or prevent use. There is no server copy, recovery, account association, or cross-device synchronization. The module fails closed on corrupt or unsupported bytes and does not intentionally overwrite the last stored value after a rejected save.

Saving or deleting a kit and importing or exporting custom-topic text make no NonStopTalk API, model-provider, product-analytics, D1, Analytics Engine, or Durable Object request. Import parses the selected file into the custom-topic editor only; it does not modify room state until the host explicitly chooses to use the custom list. Export validates and canonicalizes that editor into `nonstoptalk-topics.txt`; once downloaded, the file is outside NonStopTalk storage and cannot be removed by an in-app delete. These text controls do not import or export the whole kit library. The setup-kit store is separate from local and optional cloud coaching history, so deleting coaching history does not delete game setup kits.

Apply is a separate, explicit room mutation. The browser sends the selected duration, silence, rounds, topic pack, and topics—but not the local kit name—through one existing same-origin `apply-setup-kit` action. The room Durable Object authorizes the current host/setup phase, normalizes bounded settings, replaces a built-in pack with its server-canonical topic list or validates a nonempty normalized custom list, resets deck state, and commits one atomic room version/topic-generation change. An invalid topic source, authorization failure, or phase failure changes none of the room. The selected settings/topics then have the room's existing Durable Object persistence; the Apply action creates no setup-kit row or backup in D1, emits no Analytics Engine event, and makes no model/provider call. Later ordinary room milestones can include duration, rounds, and topic-pack ID in HMAC-pseudonymous D1 room facts under the existing telemetry policy; kit names and custom-topic text remain excluded. Public guest state continues to omit the undrawn topic list.
...
Progress exposes a point-in-time local artifact readout derived from the same fail-closed reconciliation that enforces retention. It reports exact aggregate recording-Blob plus UTF-8 transcript bytes, the 128 MiB NonStopTalk app limit, migration-grace count, earliest deadline, and per-attempt byte/deadline policy metadata. It never reads transcript text for display, never sends the snapshot over the network, and does not call browser-capacity APIs or claim to know the browser's separate quota. Existing valid artifacts are not evicted to admit a new one; the browser can still remove site data earlier, and deadline cleanup runs only on a later coaching-storage access.
...
The in-progress cloud store is separate. It keeps only explicitly backed-up compact summaries and versioned consent records in D1, keyed by a hashed anonymous browser identity. Cloud use refreshes one UTC-day-bucketed device lease; scheduled cleanup removes expired anonymous detail in bounded batches and leaves any remaining backlog for a later cron run. One global schema-v5 heartbeat records only successful scheduled/completed times and a backlog bit; public status reduces that state to `ready`, `stale`, or `backlog` and exposes no timestamps, deletion counts, or user data. Schema v6 also defines a room-milestone receipt table limited to opaque lowercase 256-bit event IDs/payload hashes and canonical UTC receipt/application/exact-90-day-expiry timestamps. The exact outbox consumer invokes its strict internal receiver for normal production and staging milestone delivery, and bounded cleanup removes expired receipts only when the database marker is 6. Marker-5 cleanup never prepares receipt-table SQL.

Release A contains the rollback-compatible local consumer, and Release B adds the normal-room producer behind exact lowercase `ROOM_MILESTONE_DELIVERY_MODE=outbox`. The committed production and staging configurations select that exact mode, and both completed their separate attended exact-mode proofs on 2026-09-02. A room mutation atomically stores its canonical aggregate-only event group in the room's private Durable Object SQLite database; the consumer drains the FIFO head one event per alarm through persisted bounded retry and privacy-minimal dead-letter state. The canonical queued payload excludes the raw room code, names, topics, member/authentication tokens, IP data, audio, transcripts, and coaching content; terminal dead letters retain only bounded reason/milestone/attempt/timing metadata, not the event ID or payload. Each delivered milestone creates one opaque D1 idempotency receipt that expires exactly 90 days after its first receiver timestamp. Staging passed the aggregate-only rollback/drain/restoration proof, and the subsequent quiet-window production canary observed the exact seven-receipt, one-room-fact, six-rollup delta. That aggregate evidence is not cryptographic per-event attribution. Exact operation requires a cryptographically random room-fact key; if that key becomes unavailable, the receiver still commits a receipt and eligible aggregate rollup but skips the room fact, and restoring the key cannot backfill that fact from a duplicate replay. The Progress delete control clears local stores and, when cloud backup is enabled and reachable, the current anonymous browser's D1 summaries. There is still no account, recovery flow, app-level encryption, or cross-device synchronization.

Product analytics are coarse aggregates, not a copy of coaching progress. Server-authoritative room milestones and accepted summary-save/delete/consent transitions produce small daily D1 increments and Analytics Engine write attempts. In production and staging, exact outbox applies only to room milestones: it atomically stores a canonical aggregate-only event with the room mutation, receipt-gates its eligible D1 effects, retries bounded failures, and makes an exact replay idempotent. Progress/consent D1 events remain best-effort through `waitUntil` and can be missed. Analytics Engine remains best-effort in both environments and gets at most one post-commit write opportunity for a newly applied room-milestone receipt; a failure or interruption can lose that point, and replay does not retry it. Status therefore reports `durable-outbox` only for the room-milestone lane when exact mode, schema 6, and the secure room-fact key are ready; an unready exact configuration is `degraded-outbox`. Aggregate event values may include turn/session duration or completed-turn counts. Separate aggregate D1 model-usage rows enforce the external-topic daily budget; they contain no theme or generated topic text. Analytics and usage counters exclude names, IP addresses, user agents, raw browser tokens, room-member tokens, audio, captured transcripts, word patterns, advice, and delivery-quality measurements such as speaking ratio. The protected admin API queries D1 rollups, but those rows remain operational rather than audit or billing truth.
...
Private/local storage does not mean risk-free storage. Anyone with access to the same unlocked browser profile may be able to view Progress, inspect setup-kit names/custom topics, or download retained recordings/transcripts; exported/downloaded files may be accessible elsewhere on the device. A shared-device demonstration should keep full retention off unless needed and delete coaching history, game setup kits, browser site data, and any downloaded files as applicable. Those controls are separate: deleting coaching history does not delete setup kits, and no in-app deletion can recall an exported file.
...
- The Cloudflare edition stores multiplayer game state in a private SQLite-backed Durable Object for up to 30 idle days. It may store bounded transcript-free judge settings, pending-review coordination, and normalized verdicts, but never a judge transcript. Coaching summaries and full artifacts never enter that object.
- The central D1 platform store holds only the allowlisted records described above: anonymous device ownership/expiry, consented compact summaries, consent records, 90-day HMAC-pseudonymous room facts, daily analytics aggregates, aggregate daily topic-provider usage budget counters, and schema-v6 milestone receipts. Production and staging exact mode insert one expiring receipt per delivered room milestone, and scheduled schema-6 cleanup removes expired rows. Receipt fields cannot contain room codes, names, topics, member/authentication tokens, IP data, audio, transcripts, or coaching content. Themes, generated topic text, judge transcript/verdict fields, raw room codes, and the HMAC key are not stored in D1; a later ordinary room fact may still contain the room's aggregate total score after a bonus.

The Cloudflare operator controls which topic-generation routine option is enabled, whether topic-generation Gemma escalation is available, the Z.AI/Gemini credentials, and access to the Workers AI binding. None of those settings changes the Cloudflare game's offline-only judge. The operator of a self-hosted Go instance separately controls `NONSTOPTALK_AI_PROVIDER` and its Anthropic or Z.AI credential. Players should use an instance they trust.
...
- A browser-persistence request, configurable retention window, or app-level encryption; the point-in-time logical-usage dashboard, exact 30-day artifact expiry, the 128 MiB app cap, summary-only fallbacks, and per-attempt deletion are implemented
...
Any future audio upload, transcript retention, transcript delivery to an external provider, account-based synchronization, human sharing, or other external-model feature requires a separate explicit consent and retention design. It must not silently inherit permission from microphone analysis, the Cloudflare game's one-turn memory-only judge consent, coaching transcript analysis, local artifact retention, or compact-summary backup.

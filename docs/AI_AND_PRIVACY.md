...
The in-progress cloud store is separate. It keeps only explicitly backed-up compact summaries and versioned consent records in D1, keyed by a hashed anonymous browser identity. Cloud use refreshes one UTC-day-bucketed device lease; scheduled cleanup removes expired anonymous detail in bounded batches and leaves any remaining backlog for a later cron run. One global schema-v5 heartbeat records only successful scheduled/completed times and a backlog bit; public status reduces that state to `ready`, `stale`, or `backlog` and exposes no timestamps, deletion counts, or user data. Schema v6 also defines an empty, dormant room-milestone receipt table limited to opaque lowercase 256-bit event IDs/payload hashes and canonical UTC receipt/application/exact-90-day-expiry timestamps. The current bridge never reads or writes it, and no receiver, outbox, retry, or receipt cleanup is active. The Progress delete control clears local stores and, when cloud backup is enabled and reachable, the current anonymous browser's D1 summaries. There is still no account, recovery flow, app-level encryption, or cross-device synchronization.

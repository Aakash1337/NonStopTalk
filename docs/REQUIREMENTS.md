...
| Reliability | Go unit/handler tests, Cloudflare rules/platform tests, 33 deterministic coaching/loop tests, cloud-progress tests, and Playwright smoke flows cover core paths—including baseline/retry resume, comparison, legacy grouping, and artifact-only deletion. Microphone calibration and optional local transcription still depend on browser, language pack, device, and room conditions. |

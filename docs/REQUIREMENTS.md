...
- Deterministic acoustic rules show at most one sparse live tip and separately select a post-attempt strength/focus. Local retrieval supplies the top card's prewritten drill; deterministic template assembly appends a priority-specific comparison sentence, and the review exposes the card's source.
- Transcript analysis is optional. It starts only after consent and only when the browser exposes mandatory local-processing support; failures preserve the acoustic review without a remote-recognition fallback.
- Transcript text is capped at 20,000 characters in memory and used to estimate word count, words per minute, filler patterns, and immediate repeated words. The compact summary retains bounded derived filler/repetition patterns after transcript consent; the full transcript is discarded by default.
- Full-session retention is a separate, unchecked setup choice. When enabled in a `MediaRecorder`-capable browser, the attempt recording and any successfully captured local transcript are stored in the separate `session-artifacts` IndexedDB store for this origin and browser profile.
- `/progress` stores and displays compact summaries, exports summary JSON without audio or full transcripts, and exposes individual recording/transcript downloads only when those artifacts exist.
...
| Reliability | Go unit/handler tests, Cloudflare rules tests, 20 deterministic coaching-engine tests, and Playwright smoke flows cover core paths, including zero/missing callback evidence, storage migration/download boundaries, and calibration/active-input stalls. Microphone calibration and optional local transcription still depend on browser, language pack, device, and room conditions. |
...
| Privacy | Coaching data remains in the browser and is not uploaded. The default path stores no attempt recording or full transcript; a separate opt-in can retain them locally. The Cloudflare game is classic-only. Local Go game transcripts are opt-in grading inputs; any configured external provider remains part of that edition's trust boundary. |
...
3. Ending or completing the attempt produces explainable evidence, a voice/quiet/unobserved timeline, one rule-selected strength/focus, one retrieved-card-plus-comparison drill, and that card's visible source.
4. When strict local recognition is absent or fails, the attempt completes without transcript-derived metrics or a remote fallback.
5. The default saved `/progress` summary contains aggregate metrics/advice and consented derived word patterns but no audio, raw sample frames, or full transcript.
6. Full-session retention starts unchecked; when selected, a recording and any available full transcript are saved only in the separate local artifact store and receive individual download controls.

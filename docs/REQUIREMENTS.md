...
21. Production and staging exact-mode activation require the Release-B code, schema 6, and a cryptographically random room-fact key to report ready. The prerequisite staging rollback/drain/restoration proof and repeated exact smoke completed on 2026-09-02. Production activation is a separate reviewed full-version release and is not complete until its target-locked attended canary passes with a verified immediate rollback version. The rollout avoids intentional version/configuration traffic splits even though both unavoidable propagation-skew directions are covered, and Release A is the minimum code rollback once any exact-mode local row may exist.

...
- Schema v6 adds a constrained `room_milestone_receipts` table and expiry index. A strict internal schema-6 receiver can receipt-gate one canonical D1 application, and bounded scheduled cleanup removes expired receipts only on marker 6. Marker-5 cleanup never prepares receipt-table SQL. Release A imports the receiver and can lazily recognize/drain an already-existing version-1 local Durable Object outbox from a later producer release or rollback. Its dormant consumer handles only the FIFO head, one event per alarm, with persisted bounded retry and privacy-minimal dead-letter state. Production and staging remain `best-effort`: ordinary rooms never initialize that local schema, enqueue events, or create receipts. Normal-room producer hookup and an end-to-end durable-delivery guarantee remain unimplemented. Receipt fields are limited to opaque lowercase 256-bit IDs/hashes and canonical UTC receipt/application/exact-90-day-expiry timestamps.
...
The slice is modular and designed around Workers, Durable Objects, D1, Static Assets, Analytics Engine, and observability on free or low-cost allocations. The schema-v4 mapping adds only two small D1 metadata rows per browser; schema v5 adds one global row and one write per successful daily cleanup. Schema v6 adds one D1 table/index, and the Release-A local consumer adds no Cloudflare service, binding, secret, provider call, paid product, or user row; the D1 marker remains 6. Its internal receiver can write when explicitly called or reached through a pre-existing local-outbox drain, while cleanup can delete expired receipts; normal room traffic adds neither a local outbox table nor a receipt write. Visible profiles, accounts/authentication, cross-device access, external coaching AI, Queues, and R2 are not implemented. A future bilateral numeric-code linking flow must use a separate `IDENTITY_HASH_KEY`, require explicit consent on both browsers, and preserve the independent cloud-summary consent boundary. The topic-only providers do not perform coaching or transcript analysis.
...
| Reliability | Go unit/handler tests, Cloudflare rules/platform tests, a shared versioned six-family game-rule contract, 34 deterministic coaching/loop tests, cloud-progress tests, and Playwright smoke flows cover core paths—including rejected-action atomicity, a two-browser Cloudflare WebSocket game with isolated identities and host-authorization rejection, baseline/retry persistence gating/resume, comparison, legacy grouping, artifact-only deletion, and cleanup-heartbeat failure/backlog/staleness behavior. Microphone calibration and optional local transcription still depend on browser, language pack, device, and room conditions. |
...
13. Applying schema v4 backfills and maintains a one-browser/one-profile mapping without changing device-scoped session create/list/export/delete behavior, API payloads, or consent choices.
14. Applying schema v5 preserves all user tables, initializes one cleanup grace heartbeat, advances it only after a successful bounded cron, and degrades public status after 36 hours or while backlog remains.
15. Applying schema v6 preserves all schema-v5 records and compatibility queries, creates the constrained empty receipt table, advances the marker to 6, and leaves the 5/6 bridge functional without activating normal room delivery.
16. On schema 6, a canonical internal delivery applies its receipt-gated D1 effects once; an exact replay does not repeat them, an event-ID/payload conflict is rejected, and Analytics Engine receives at most one best-effort post-commit opportunity. No public route or ordinary best-effort room delivery invokes this primitive; only explicit internal/test use or a pre-existing local-outbox drain does.
17. Scheduled cleanup removes expired receipts in bounded schema-6 batches and includes remaining receipt work in its shared backlog result; marker-5 cleanup never prepares receipt-table SQL.
18. With production/staging in `best-effort`, an ordinary room creates no local outbox schema or D1 receipt. A pre-existing valid version-1 local outbox is drained FIFO one event per alarm with persisted bounded retry/dead-letter state, and switching back to `best-effort` does not disable that drain.
...
- Normal-room production into the implemented version-1 Durable Object SQLite outbox and activation of the internal receipt receiver; Release A contains only the dormant FIFO retry/dead-letter rollback consumer

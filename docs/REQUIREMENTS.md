...
20. Staging exact-mode activation occurs only after the dormant Release-B code is fully deployed and schema 6 plus a cryptographically random room-fact key report ready. Production exact-mode activation additionally requires the pending staging rollback/drain/restoration proof and a repeated healthy exact smoke. The rollout avoids intentional version/configuration traffic splits even though both unavoidable propagation-skew directions are covered, and Release A is the minimum code rollback once any exact-mode local row may exist.

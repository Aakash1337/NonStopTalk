...
- Schema v6 adds a constrained `room_milestone_receipts` table and expiry index. A strict internal schema-6 receiver can receipt-gate one canonical D1 application, and bounded scheduled cleanup removes expired receipts only on marker 6. Marker-5 cleanup never prepares receipt-table SQL. The receiver is not wired to normal rooms, so ordinary room traffic creates no receipts and still uses best-effort analytics fan-out. There is no Durable Object outbox, retry, dead-letter path, or end-to-end durable-delivery guarantee. Receipt fields are limited to opaque lowercase 256-bit IDs/hashes and canonical UTC receipt/application/exact-90-day-expiry timestamps.
...
The slice is modular and designed around Workers, Durable Objects, D1, Static Assets, Analytics Engine, and observability on free or low-cost allocations. The schema-v4 mapping adds only two small D1 metadata rows per browser; schema v5 adds one global row and one write per successful daily cleanup. Schema v6 adds one table/index and no service, binding, secret, provider call, or user row. Its internal receiver can write only when explicitly called, while cleanup can delete expired receipts; normal room traffic adds no receipt write. Visible profiles, accounts/authentication, cross-device access, external coaching AI, Queues, and R2 are not implemented. A future bilateral numeric-code linking flow must use a separate `IDENTITY_HASH_KEY`, require explicit consent on both browsers, and preserve the independent cloud-summary consent boundary. The topic-only providers do not perform coaching or transcript analysis.
...
16. On schema 6, an explicit canonical internal delivery applies its receipt-gated D1 effects once; an exact replay does not repeat them, an event-ID/payload conflict is rejected, and Analytics Engine receives at most one best-effort post-commit opportunity. No public or normal room path invokes this primitive.
...
- A Durable Object SQLite outbox, alarm retry, dead-letter lifecycle, and normal-room hookup to the implemented internal receipt receiver

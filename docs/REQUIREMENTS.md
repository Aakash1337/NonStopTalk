...
- Browser support for mandatory on-device `SpeechRecognition.processLocally` only for optional transcript-assisted coaching or the optional game judge; acoustic coaching and classic/manual play remain available without it
...
- A cryptographically random `ROOM_FACT_HASH_KEY` of 32–1024 UTF-8 bytes for healthy exact room-milestone outbox mode; numeric-only output from a cryptographic generator is valid
...
- Production and staging exact outbox change only room milestones: their eligible D1 effects are retried and receipt-gated. Progress/consent D1 events in both environments and every Analytics Engine write remain best-effort. D1 supplies the protected admin readout, but neither sink is audit or billing truth.
- Schema v6 adds a constrained `room_milestone_receipts` table and expiry index. A strict internal receiver receipt-gates one canonical D1 application, and bounded scheduled cleanup removes expired receipts only on marker 6. Marker-5 cleanup never prepares receipt-table SQL. Release A can lazily recognize and drain a valid version-1 local Durable Object outbox FIFO, one head per alarm, with persisted bounded retry and privacy-minimal dead-letter state. Release B adds a normal-room producer behind exact lowercase `ROOM_MILESTONE_DELIVERY_MODE=outbox`: a milestone mutation atomically commits room state, stable lifecycle metadata, the complete ordered event group, and the shared alarm before response or broadcast. Expected capacity/canonicalization drops preserve gameplay and record bounded counters; other local storage or alarm failures roll back the mutation. The committed production and staging configurations select exact `outbox`; both reported healthy `durable-outbox` during their separate 2026-09-02 activation proofs, while the read-only monitor enforces that policy for current deployments. Each delivered milestone creates one opaque D1 idempotency receipt with exact 90-day expiry. Staging passed its rollback/drain/restoration proof plus repeated exact smoke on 2026-09-02; production then passed its separate attended quiet-window activation canary with the exact seven-receipt, one-room-fact, six-rollup delta. Receipt fields are limited to opaque lowercase 256-bit IDs/hashes and canonical UTC receipt/application/exact-90-day-expiry timestamps.
- One scheduled/manual, bounded, read-only workflow probes production and staging as independent matrix rows without credentials, dependency installation, or mutations. It requires both current deployments to report `durable-outbox`, so an otherwise healthy delivery-policy drift fails visibly. It recursively exact-checks all 12 currently deployable `.js`/`.mjs` files from `cloudflare/public`; `.assetsignore` excludes the nine current exact case-insensitive `*.test.js` and `*.test.mjs` paths from both deployments, and each live row verifies those excluded paths are not executable.
- Room authority remains in Durable Objects. D1 and analytics failures must not break live rooms or local coaching history.
- Raw audio and browser-encoded recordings always remain in the browser. Coaching captured text also remains there. The only game-text exception is a speaker-consented, capped current-turn transcript sent once to the same-origin Cloudflare Worker for memory-only offline grading; it is never stored in Durable Objects, D1, logs, analytics, history, or a provider. Optional coaching D1 backup contains only the compact allowlisted summary.
...
The slice is modular and designed around Workers, Durable Objects, D1, Static Assets, Analytics Engine, and observability on free or low-cost allocations. The schema-v4 mapping adds only two small D1 metadata rows per browser; schema v5 adds one global row and one write per successful daily cleanup. Schema v6 adds one D1 table/index, while the Release-A consumer, Release-B producer, and production activation add no Cloudflare service, binding, secret, migration, provider/model call, paid product, fixed monthly cost, or user row; the D1 marker remains 6. Production and staging exact mode reuse bounded private Durable Object SQLite and its existing alarm, require the already-documented room-fact HMAC secret, and add one 90-day D1 receipt per delivered milestone. The Cloudflare J1 judge similarly reuses the existing browser/Worker/room path and adds no D1 state or migration, key/secret, binding, provider/model call, paid API, Queue, or fixed monthly cost. Visible profiles, accounts/authentication, cross-device access, external coaching AI, Queues, and R2 are not implemented. A future bilateral numeric-code linking flow must use a separate `IDENTITY_HASH_KEY`, require explicit consent on both browsers, and preserve the independent cloud-summary consent boundary. The current Cloudflare topic providers do not perform coaching or judge/transcript analysis.
...
- The host can configure speaking duration, silence limit, rounds, topic source, and whether the optional judge is available. The Cloudflare judge defaults off.
...
### Optional AI judge — implemented in both editions
...
- The transcript is capped, used only for current-turn grading, and not persisted in room or history state.
- In Cloudflare J1, the browser sends at most 8 KiB of text once to the same-origin Worker after the exact speaker consents. It is held only in browser/Worker request memory and never enters Durable Object SQLite, D1, logs/traces, Analytics Engine, Web Analytics, history, model-usage accounting, or a provider. Room state contains only bounded transcript-free pending metadata and normalized verdicts.
- Cloudflare classic scoring commits first and its baseline is never removed. The deterministic offline verdict can add its bonus to that exact turn once; while it is pending, standings/winner are provisional. Missing text, unsupported recognition, stale/duplicate authorization, invalid output, or a review that does not commit before its deadline adds no bonus. If a completion response is lost after commit, authoritative room state confirms whether the one-time bonus was applied.
- Cloudflare J1 adds no paid or model API, key, secret, binding, D1 row/migration, Queue, or provider budget. Its existing GLM-4.7 Flash, GLM-5.3 Flash, and Gemma 4 31B adapters remain topic-generation-only. Any future remote judge requires a new schema/version gate and separate external-processing consent/cost/privacy rollout.
- In the Go edition, `NONSTOPTALK_AI_PROVIDER=offline|anthropic|glm` selects the local judge and theme generator. Unset preserves legacy Anthropic selection only when `ANTHROPIC_API_KEY` exists; otherwise it stays offline, and `ZAI_API_KEY` alone does not opt in. Explicit providers require their matching key; invalid or incomplete selections warn and fail closed to the offline heuristic.
- After the Go speaker's per-turn consent, Anthropic or Z.AI GLM receives only the assigned topic and capped transcript as user data, never microphone audio or room metadata. Local theme generation sends an external provider only the theme as user content.
...
- The Cloudflare edition rate-limits room creation per source connection and applies the general 60-requests-per-minute `API_RATE_LIMITER` `platform` bucket to both model routes. Only the topic route then uses the five-requests-per-minute `MODEL_RATE_LIMITER` under its `model-topics` scope; the deterministic offline judge does not consume that provider limiter. It rejects cross-origin mutations/WebSocket upgrades, caps live sockets per member and room, authorizes topic generation against the current host/setup phase, and authorizes judge work against the exact pending speaker/turn.
...
| Privacy | Audio samples and recordings remain local. Coaching captured text remains local; the Cloudflare game's only text exception is a separately consented, capped current-turn transcript sent to the same-origin Worker for memory-only offline grading and never persisted, logged, sent to analytics, or sent to a provider. Default/off coaching makes no coaching-data API request; explicit backup sends only allowlisted summary fields. The saved Cloudflare setup-kit library remains in unencrypted origin/browser-local `localStorage`, unsynced and without recovery; local save/delete/import/export is uninstrumented and makes no application network request. Applying a kit sends its selected settings/topics only to the existing room Durable Object and performs no D1, Analytics Engine, model-provider, or cloud-backup work in that action. A later ordinary room milestone may include the applied duration, rounds, and topic-pack ID in HMAC-pseudonymous D1 room facts under the existing telemetry policy; kit names and custom-topic text remain excluded. Cloudflare external topic generation requires per-attempt host consent; the normalized theme, capped at 200 characters, is the only host or room content sent to a provider. Product analytics and topic-provider budget state are coarse aggregates rather than content records. The local Go edition's separately consented external AI-judge path retains its documented provider boundary. |
...
11. With the Cloudflare judge enabled, the exact speaker can separately consent to strict on-device transcription for one microphone turn; only the capped text reaches the same-origin Worker's offline heuristic. Classic score lands first and its baseline remains safe; a review that does not commit adds no bonus, and authoritative room state resolves an ambiguous completion response. Audio and transcript text never enter Durable Object/D1 storage, logs, analytics, history, or a provider.
12. The local Go app and native Cloudflare edition both provide the documented core game flow.
...
21. Production and staging exact-mode activation require the Release-B code, schema 6, and a cryptographically random room-fact key to report ready. The staging rollback/drain/restoration proof, repeated exact staging smoke, and separate target-locked production canary all completed on 2026-09-02 with a verified Release-A-compatible rollback floor. The rollout avoided intentional version/configuration traffic splits even though both unavoidable propagation-skew directions are covered, and Release A is the minimum code rollback once any exact-mode local row may exist. Routine exact-to-exact releases use read-only health probes and do not repeat the mutating activation canary.
22. The production and staging health rows recursively derive the complete deployable `.js`/`.mjs` manifest from `cloudflare/public`—currently 12 deployed modules and nine excluded test modules—validate one bounded release generation, and require exact checked-out bytes, canonical UTF-8 without a BOM, JavaScript MIME, `nosniff`, a non-HTML bounded body, and valid module syntax for every deployed asset. The deployment contract keeps exact case-insensitive `*.test.js` and `*.test.mjs` paths excluded through `.assetsignore`, each live row requires those excluded paths to resolve to protected HTML rather than executable JavaScript, and the existing core SPA graph receives additional import/consumption/export checks.
...
- Remote-provider judge parity between the Go and Cloudflare editions beyond the implemented Cloudflare offline-judge, topic-draft, setup-kit, microphone-selection, and sound-cue slices; any GLM/Gemma Cloudflare judge remains schema-gated future work

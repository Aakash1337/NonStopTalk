...
| Reliability | Go unit/handler tests, Cloudflare rules/platform tests, 34 deterministic coaching/loop tests, cloud-progress tests, and Playwright smoke flows cover core paths—including baseline/retry persistence gating/resume, comparison, legacy grouping, artifact-only deletion, and cleanup-heartbeat failure/backlog/staleness behavior. Microphone calibration and optional local transcription still depend on browser, language pack, device, and room conditions. |

...
- One scheduled/manual, bounded, read-only workflow probes production and staging as independent matrix rows without credentials, dependency installation, or mutations. It requires both current deployments to report `durable-outbox`, so an otherwise healthy delivery-policy drift fails visibly. It recursively exact-checks the complete deployable `.js`/`.mjs` generation from `cloudflare/public`; `.assetsignore` excludes exact case-insensitive `*.test.js` and `*.test.mjs` paths from both deployments, and each live row verifies those excluded paths are not executable.
...
22. The production and staging health rows recursively derive the complete deployable `.js`/`.mjs` manifest from `cloudflare/public`, validate one bounded release generation, and require exact checked-out bytes, canonical UTF-8 without a BOM, JavaScript MIME, `nosniff`, a non-HTML bounded body, and valid module syntax for every asset. The deployment contract keeps exact case-insensitive `*.test.js` and `*.test.mjs` paths excluded through `.assetsignore`, each live row requires those excluded paths to resolve to protected HTML rather than executable JavaScript, and the existing core SPA graph receives additional import/consumption/export checks.

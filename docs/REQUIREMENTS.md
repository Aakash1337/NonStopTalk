...
- **Delete saved artifacts** removes one attempt's recording/captured transcript, its optional lifecycle row when present, and resets its artifact metadata in one local transaction while preserving the compact summary, explicit pair, and comparison.
...
| Reliability | Go unit/handler tests, Cloudflare rules/platform tests, a shared versioned six-family game-rule contract, 34 deterministic coaching/loop tests plus two IndexedDB storage-contract tests, cloud-progress tests, and Playwright smoke flows cover core paths—including rejected-action atomicity, a two-browser Cloudflare WebSocket game with isolated identities and host-authorization rejection, baseline/retry persistence gating/resume, comparison, legacy grouping, artifact-only deletion, storage rollback/fallback behavior, and cleanup-heartbeat failure/backlog/staleness behavior. Microphone calibration and optional local transcription still depend on browser, language pack, device, and room conditions. |
...
7. JSON export excludes full artifacts, and confirmed local-history deletion clears all local coaching stores.
...
18. In any environment configured `best-effort`, an ordinary room creates no local outbox schema or D1 receipt. A pre-existing valid version-1 local outbox is drained FIFO one event per alarm with persisted bounded retry/dead-letter state, and switching back to `best-effort` does not disable that drain.

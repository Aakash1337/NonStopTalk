...
- Schema v6 adds only an empty, constrained `room_milestone_receipts` table and expiry index. The schema-5/6 bridge does not read or write them, so no idempotent receiver, outbox, retries, cleanup, or durable-delivery guarantee is active yet. Receipt fields are limited to opaque lowercase 256-bit IDs/hashes and canonical UTC receipt/application/exact-90-day-expiry timestamps.
...
The slice is modular and designed around Workers, Durable Objects, D1, Static Assets, Analytics Engine, and observability on free or low-cost allocations. The schema-v4 mapping adds only two small D1 metadata rows per browser; schema v5 adds one global row and one write per successful daily cleanup. Schema v6 adds one empty table/index and no service, binding, secret, provider call, user row, or runtime write. Visible profiles, accounts/authentication, cross-device access, external coaching AI, Queues, and R2 are not implemented. A future bilateral numeric-code linking flow must use a separate `IDENTITY_HASH_KEY`, require explicit consent on both browsers, and preserve the independent cloud-summary consent boundary. The topic-only providers do not perform coaching or transcript analysis.
...
15. Applying schema v6 preserves all schema-v5 records and compatibility queries, creates the constrained empty receipt table, advances the marker to 6, and leaves the 5/6 bridge functional without activating receipt delivery.
...
- A receipt-gated idempotent room-milestone receiver and Durable Object outbox/retry/dead-letter lifecycle; schema v6 supplies only the unused table foundation

...
Progress can delete one attempt's saved recording/captured transcript without deleting its compact summary. The summary metadata, artifact, and required lifecycle row change in one transaction, so an explicit baseline/retry relationship and its measurement comparison remain available. IndexedDB v3 gives new artifacts exactly 30 days and gives valid v1/v2 artifacts exactly 30 days from migration. New artifacts share a 128 MiB logical cap without eviction; app-limit and browser-quota outcomes retain the compact summary only and report that the artifact was not retained. Progress shows a point-in-time artifact dashboard calculated from actual streamed Blob/transcript payloads: aggregate logical bytes, the app limit, earliest retention deadline, migration-grace state, and each attempt's bytes/deadline. It is deliberately not a browser-quota estimate. Browser storage is best-effort and may disappear earlier, and expiry cleanup occurs on a later storage access. There is no persistent-storage request or app-level encryption, and downloaded files remain outside application control.
...
`readCoachingProgressSnapshot()` reuses that streamed three-store reconciliation to return post-reconciliation summaries and a content-free local usage snapshot in the same transaction. It retains only aggregate numbers and per-session ID/time/byte policy fields after each Blob cursor step, so the dashboard does not materialize all retained recordings at once. The result distinguishes the exact app cap from unknown browser capacity, remains truthful when migrated grace content is above the cap, rechecks time after the payload scan, and reports only payload rows actually removed during that access. A newer app feature-detects this export through the module namespace, so a transient Release-B-app/Release-A-storage asset pairing keeps Progress usable and shows an unavailable dashboard instead of failing module linkage.
...
- Retained artifacts support per-attempt deletion, exact 30-day app retention, a no-eviction 128 MiB logical cap, and a point-in-time Progress dashboard for exact logical use and retention deadlines. That dashboard is not a browser-capacity estimate and is refreshed only on a coaching-storage access. IndexedDB remains best-effort: storage pressure, private mode, browser policy, site-data deletion, or database failure can remove or block data earlier. There is no persistent-storage request, configurable retention, or app-level encryption, and anyone with access to the unlocked browser profile may reach artifacts before deletion or expiry.

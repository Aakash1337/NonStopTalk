...
- Forty-one deterministic tests—34 coaching/loop tests plus seven IndexedDB storage-contract tests—and browser flows covering the single coached format, the default baseline → Progress/reload/resume → unassisted-retry format, baseline persistence gating, safe grouping/comparison, v1→v3 migration, exact lifecycle retention, per-attempt artifact deletion, Release-A/future-schema compatibility, fail-closed corruption, no-eviction cap behavior, summary-only fallbacks, and no coaching-data API request when backup is off
...
3. Add user-authored prompts/goals, stronger calibration guidance, a user-facing artifact usage/retention dashboard over the implemented automatic expiry and logical cap, and clearer signal-confidence explanations.

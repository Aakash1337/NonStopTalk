...
- Versioned Worker APIs and a central D1 database for explicitly backed-up compact coaching summaries, versioned consent, anonymous expiry, HMAC-pseudonymous room facts, and daily aggregate counters with receipt-gated production and staging room milestones
...
- A schema-v6 additive, privacy-minimal `room_milestone_receipts` table, strict internal canonical receiver, bounded receipt expiry cleanup, Release-A rollback bridge, and Release-B normal-room producer. The committed production and staging policies select exact `outbox`; both reported healthy `durable-outbox` during their separate 2026-09-02 activation proofs, and the read-only monitor requires that result from current deployments. Exact mode uses atomic state/event/alarm commits plus FIFO receipt-gated D1 delivery. Staging passed its rollback/drain/restoration proof plus repeated exact smoke on 2026-09-02, followed by the separate attended production activation canary with the exact seven-receipt, one-room-fact, six-rollup delta
- Coarse privacy-safe product events: production and staging room-milestone D1 effects are durable and replay-idempotent, progress/consent D1 transitions remain best-effort in both environments, and Analytics Engine remains best-effort everywhere; D1 supplies protected aggregate product-analytics and model-usage operational readouts, not audit or billing truth
...
- Schema-v5 cleanup heartbeat with monotonic cron timestamps, backlog/staleness readiness, and a twice-hourly zero-secret read-only matrix that independently checks production and staging `durable-outbox` policy plus the complete deployable `.js`/`.mjs` Static Assets set as one exact release generation
...
- A manual production activation canary locked to the exact origin, root Worker/D1 resources, active full-version deployment, reviewed best-effort rollback version, required secret inventory, and bounded aggregate lifecycle; its unit gate runs in CI, and its one-time attended production activation proof passed on 2026-09-02
- Recursive production/staging integrity probes for the complete deployable `.js`/`.mjs` Static Assets generation, with canonical UTF-8/no-BOM exact checked-out source, JavaScript MIME, `nosniff`, module-syntax and decompressed-size/count/path/aggregate bounds; `.assetsignore` keeps exact case-insensitive `*.test.js` and `*.test.mjs` paths out of both deployments, live negative probes require protected HTML at those paths, and core semantic graph checks and browser smokes retain their separate roles
...
- Re-pin and repeat the target-locked attended production canary only for a future high-risk delivery-policy activation or restoration; ordinary exact-to-exact releases and monitoring remain read-only and both environments require healthy exact `outbox`

...
- A schema-v6 additive, privacy-minimal `room_milestone_receipts` table, strict internal canonical receiver, bounded receipt expiry cleanup, Release-A rollback bridge, and Release-B normal-room producer. Production and staging remain `best-effort`, so ordinary rooms create neither local outbox tables nor receipts. Only exact lowercase `outbox` enables an atomic state/event/alarm commit and FIFO receipt-gated D1 delivery; activation remains an operational rollout step
...
- Full dormant Release-B propagation, staging exact-mode/rollback drill, and a single-version production activation of the implemented room-milestone outbox; both deployed environments remain intentionally `best-effort` until those checks pass, and the Release-A-compatible one-header ownership sentinel covers unavoidable propagation skew in either direction

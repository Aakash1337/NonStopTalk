...
- A schema-v6 additive, privacy-minimal `room_milestone_receipts` table, strict internal canonical receiver, bounded receipt expiry cleanup, Release-A rollback bridge, and Release-B normal-room producer. Production remains `best-effort`, so ordinary production rooms create neither local outbox tables nor receipts. Staging intentionally runs exact `outbox`, is currently healthy with `durable-outbox`, and uses atomic state/event/alarm commits plus FIFO receipt-gated D1 delivery; its rollback/drain/restoration proof remains pending
...
- Complete the pending staging rollback/drain/candidate-restoration drill and repeat the exact smoke before a separately reviewed single-version production activation; staging is currently healthy exact `outbox`, production remains `best-effort`, and the Release-A-compatible one-header ownership sentinel covers unavoidable propagation skew in either direction

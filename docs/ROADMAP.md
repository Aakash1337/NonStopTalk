...
- A schema-v6 additive, privacy-minimal `room_milestone_receipts` table, strict internal canonical receiver, bounded receipt expiry cleanup, Release-A rollback bridge, and Release-B normal-room producer. The committed production and staging policies select exact `outbox`; staging reports healthy `durable-outbox`, and production activation requires the same readiness plus the attended post-merge canary. Exact mode uses atomic state/event/alarm commits plus FIFO receipt-gated D1 delivery. Staging passed its rollback/drain/restoration proof plus repeated exact smoke on 2026-09-02 as the production-activation prerequisite
...
- Schema-v5 cleanup heartbeat with monotonic cron timestamps, backlog/staleness readiness, and a twice-hourly zero-secret read-only matrix that independently checks production and staging `durable-outbox` policy
...
- A manual production activation canary that is locked to the exact origin, root Worker/D1 resources, active full-version deployment, reviewed best-effort rollback version, required secret inventory, and bounded aggregate lifecycle; its unit gate runs in CI, but the mutating quiet-window proof is intentionally attended and remains a post-merge step
...
- Repeat the target-locked attended production canary and record a freshly verified immediate rollback version for future high-risk delivery-policy releases; ordinary monitoring remains read-only and both environments require healthy exact `outbox`

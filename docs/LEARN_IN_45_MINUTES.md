...
| Artifact lifecycle metadata | Only with a retained or migrated artifact | Required content-free `artifact-lifecycle` ledger in IndexedDB v3 | ID, exact retention/expiry times, logical bytes, schema version, and migration grace only; no media or captured words |
...
Full-session retention and compact cloud backup are independent and start off. A standalone **Try again** and direct baseline retry preserve visible selections so the user can review or uncheck them; a retry resumed from Progress starts every optional data choice unchecked. Progress can delete one attempt's artifacts while preserving its compact summary and pair. IndexedDB v3 gives a new artifact exactly 30 days from save and a valid v1/v2 artifact exactly 30 days from upgrade. Logical bytes are `Blob.size` plus exact UTF-8 transcript bytes; new artifacts share a 128 MiB cap, and the app does not evict valid unexpired artifacts. An app-limit or browser-quota outcome keeps the compact summary only. Malformed or inconsistent artifact state fails closed, while the Release-A rollback floor can continue using an already-upgraded compatible database. One UTC-day-bucketed device lease controls all of an anonymous browser's cloud summaries, lasts at least 30 and less than 31 days after cloud use, and avoids per-summary renewal writes. The cloud cookie is not an account or recovery credential, so there is no cross-device Progress. IndexedDB remains best-effort: browser policy, storage pressure, private mode, or site-data deletion can remove it sooner, expiry cleanup runs on a later access, and deleting browser data cannot remove files already downloaded.

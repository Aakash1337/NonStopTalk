...
- An in-progress modular platform layer using D1 for queryable consented summaries and protected daily rollups, plus Analytics Engine for coarse best-effort telemetry; Durable Objects remain responsible for live rooms, and a dormant-by-configuration atomic outbox producer plus its Release-A-compatible consumer can make room-side D1 effects durable without adding another service
...
- Staged activation and operational validation of the implemented normal-room outbox producer; production and staging remain best-effort until the dormant build and rollback drill are verified

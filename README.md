...
Schema v6 adds a privacy-minimal milestone-receipt table, a strict internal canonical receiver, and bounded receipt expiry cleanup. The Release-B Worker also contains a normal-room producer behind exact `ROOM_MILESTONE_DELIVERY_MODE=outbox`. The committed production and staging configurations select that mode; staging is already verified, while production activation still requires the attended post-merge canary. For a real milestone it commits the room JSON, stable lifecycle metadata, the complete ordered event group, and the shared alarm in one Durable Object transaction; capacity or canonicalization drops keep gameplay available and never fall back to the legacy path. The FIFO consumer uses persisted bounded retry and privacy-minimal dead letters, while one 90-day D1 receipt per delivered milestone makes the eligible D1 room-fact and daily-rollup effects idempotent. This reuses bounded room-local SQLite and its existing alarm and adds no service, binding, secret, migration, model, paid product, or fixed monthly cost. The 2026-09-02 staging drill proved a pending Release-B joined event drained exactly once after rollback to Release A, proved the Release-A legacy control, restored the candidate, and repeated the exact seven-receipt/one-room-fact/six-rollup smoke. Release A is the minimum rollback floor after any activation because it can drain Release-B rows. Cleanup reads/deletes receipts only on schema 6, while schema-5 cleanup never prepares receipt-table SQL. Analytics Engine still receives only one best-effort post-D1-commit opportunity for a newly applied receipt, with no retry on failure or replay.

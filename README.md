...
Schema v6 adds a privacy-minimal milestone-receipt table, a strict internal canonical receiver, and bounded receipt expiry cleanup. Production and staging still use `ROOM_MILESTONE_DELIVERY_MODE=best-effort`: ordinary rooms pass milestone headers to the top-level Worker's `waitUntil` fan-out, create neither local outbox tables nor D1 receipt rows, and have no end-to-end durable-delivery guarantee. This compatibility release imports the receiver and can lazily recognize and drain an already-existing version-1 Durable Object outbox left by a later producer release or rollback. That dormant bridge processes the FIFO head one at a time with persisted, bounded retry and privacy-minimal dead-letter state; it does not enqueue normal room events. Cleanup reads/deletes receipts only on schema 6, while schema-5 cleanup never prepares receipt-table SQL. Analytics Engine still receives only one best-effort post-D1-commit opportunity for a newly applied receipt, with no retry on failure or replay. Normal-room outbox production and activation remain future work.

Raw microphone samples, browser-encoded recordings, and captured transcript text always stay in the browser. Live rooms remain in per-room Durable Objects; D1 is the queryable platform store, and aggregate analytics intentionally exclude names, IP addresses, room-member tokens, audio, transcript text, and per-person identifiers. D1 daily rollups and Analytics Engine delivery are both best-effort for ordinary traffic: failures do not break rooms or coaching, and progress responses no longer wait for these telemetry writes. Protected admin readouts expose aggregate product analytics and model usage for operations, not audit or billing truth. `/api/v1/admin/model-usage` returns only global/daily call, outcome, provider/model/task, input/output/total/cached-input/reasoning token, and latency aggregates—never prompts, responses, room data, identities, or authentication tokens. A timed-out provider call is counted even when it returns no usage; its token totals can therefore undercount vendor-billed work. The public status route checks D1 and reports non-secret configured or degraded platform capabilities, including cleanup health. Release A always reports analytics `delivery: best-effort`, even if its config is accidentally changed; configuration alone cannot overstate a dormant consumer as an active producer. A future producer release may report `durable-outbox` only after exact `outbox` mode plus schema-6/readiness gates. Accounts and cross-device authentication, external coaching AI, Queues, and R2 are future options, not parts of this slice.
...
- An in-progress modular platform layer using D1 for queryable consented summaries and protected best-effort daily rollups, plus Analytics Engine for coarse best-effort telemetry; neither analytics path is audit or billing truth, Durable Objects remain responsible for live rooms, and a dormant local-outbox consumer provides a future rollback floor without changing ordinary delivery
...
- Normal-room production into the implemented version-1 Durable Object outbox and activation of receipt-gated delivery; the current compatibility bridge only drains rows that already exist

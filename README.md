...
- Origin-scoped IndexedDB v2 for compact coaching summaries—including explicit loop/baseline/role metadata—and a separate, explicitly opted-in recording/captured-transcript store; exports exclude those artifacts, per-attempt deletion preserves the compact record, and full local deletion clears all local coaching stores. Storage operations are isolated in `coach-storage.js`; this v2 release can safely reopen a compatible newer database as a rollback floor, maintains optional future lifecycle bookkeeping only when that store already exists, and honors expirations recorded there

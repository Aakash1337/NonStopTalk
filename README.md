...
- A Cloudflare-SPA coaching prototype at `/practice`: 30–90 second interview, presentation, or impromptu attempts; browser-side audio measurements; an explicit review-only baseline → review → unassisted-retry loop by default; an alternative single coached attempt with sparse deterministic live tips; and evidence-based review. Four-second microphone calibration starts medium/high-confidence attempts directly, while low-confidence calibration pauses before the attempt so the user can retry on the already-connected microphone, continue with visibly limited evidence, or cancel. Recording and optional local transcription do not start before that choice. Review explains calibration, coverage, analyzed time, observed voice, callback continuity, and the specific reasons that limited measurement confidence; low calibration keeps attempt confidence low and therefore keeps any pair comparison limited.
...
- A separate, off-by-default retention choice that can keep the attempt recording and any locally captured transcript in the same origin-local IndexedDB database; recognition gets up to two seconds to flush at finish, and a timeout or error after text was captured preserves it but marks it as possibly partial in Review and Progress. IndexedDB v3 gives every new artifact exactly 30 days, gives artifacts migrated from an earlier schema exactly 30 days from the upgrade, and tracks only ID/time/logical-byte policy metadata in a required content-free lifecycle ledger. Progress shows a point-in-time local dashboard computed from the actual recording Blob and UTF-8 transcript bytes, including the 128 MiB app limit, earliest retention deadline, migration-grace state, and each retained attempt's bytes/deadline. This is not a browser-quota estimate and makes no network request. JSON export excludes artifacts. Each attempt's artifacts can be deleted while its compact summary and comparison remain, and full history deletion clears all local coaching stores
...
- One dependency-free, browser-local microphone choice shared by Cloudflare Practice and the current Play turn driver, plus local voice-activity/silence detection and a manual timer fallback. The choice stores only the browser's opaque device ID, never its label, and makes no application request. Both editions also provide short Web Audio game cues with a persistent browser-local opt-out; online cues run only for the current local turn driver and make no application or analytics request
- Classic scoring, score explanations, host adjustments, standings, winner view, and the last 20 finished games in each room. Both editions offer an optional relevance bonus after room-host enablement and fresh consent from the exact speaker for that turn. The Cloudflare edition's first judge slice is an offline deterministic heuristic; its classic score lands first and remains the fallback
...
Schema v6 adds a privacy-minimal milestone-receipt table, a strict internal canonical receiver, and bounded receipt expiry cleanup. The Release-B Worker also contains a normal-room producer behind exact `ROOM_MILESTONE_DELIVERY_MODE=outbox`. The committed production and staging configurations select that mode, and both environments completed their attended exact-mode proofs on 2026-09-02. For a real milestone it commits the room JSON, stable lifecycle metadata, the complete ordered event group, and the shared alarm in one Durable Object transaction; capacity or canonicalization drops keep gameplay available and never fall back to the legacy path. The FIFO consumer uses persisted bounded retry and privacy-minimal dead letters, while one 90-day D1 receipt per delivered milestone makes the eligible D1 room-fact and daily-rollup effects idempotent. This reuses bounded room-local SQLite and its existing alarm and adds no service, binding, secret, migration, model, paid product, or fixed monthly cost. The staging drill proved a pending Release-B joined event drained exactly once after rollback to Release A, proved the Release-A legacy control, restored the candidate, and repeated the exact seven-receipt/one-room-fact/six-rollup smoke. The separate quiet-window production canary then observed those same exact aggregate deltas. Release A is the minimum rollback floor after activation because it can drain Release-B rows. Cleanup reads/deletes receipts only on schema 6, while schema-5 cleanup never prepares receipt-table SQL. Analytics Engine still receives only one best-effort post-D1-commit opportunity for a newly applied receipt, with no retry on failure or replay.

Coaching microphone samples, browser-encoded recordings, and captured transcript text stay in the browser. The separate optional Cloudflare game judge sends only the consenting speaker's capped on-device transcript to the same-origin Worker for in-memory grading; audio never uploads, and the transcript never enters a Durable Object, D1, logs, analytics, or a model provider. Live rooms remain in per-room Durable Objects; D1 is the queryable platform store, and aggregate analytics intentionally exclude names, IP addresses, room-member tokens, audio, transcript text, and per-person identifiers. The committed production and staging exact-outbox policy provides durable, replay-idempotent room-milestone D1 effects when its readiness gates pass. Progress/consent D1 counters remain best-effort in both environments, and Analytics Engine stays best-effort everywhere. Protected admin readouts expose aggregate product analytics and model usage for operations, not audit or billing truth. `/api/v1/admin/model-usage` returns only global/daily call, outcome, provider/model/task, input/output/total/cached-input/reasoning token, and latency aggregates—never prompts, responses, room data, identities, or authentication tokens. A timed-out provider call is counted even when it returns no usage; its token totals can therefore undercount vendor-billed work. The public status route checks D1 and reports non-secret configured or degraded platform capabilities, including cleanup health. Delivery is `best-effort` for every non-exact mode, `durable-outbox` only for the compiled producer plus exact `outbox`, schema 6, and a secure room-fact key, and `degraded-outbox` when exact mode is requested without those readiness gates. Accounts and cross-device authentication, external coaching AI, Queues, and R2 are future options, not parts of this slice.
...
The platform also includes a narrow, disabled-by-default theme-to-topics provider boundary. The host must explicitly approve each external generation attempt. The only host or room content sent to a provider is that request's normalized theme, capped at 200 characters: never audio, transcript text, names, room authorization data, or a room token. `TOPIC_ROUTINE_PROVIDER=glm` selects direct Z.AI GLM-4.7-Flash with `ZAI_API_KEY`; `TOPIC_ROUTINE_PROVIDER=glm53` selects the `AI` binding's `@cf/zai-org/glm-5.3-flash` model without a vendor key. GLM-4.7 is the strict-free choice, while GLM-5.3 is the preferred cheap model when Workers Paid is acceptable. `TOPIC_ESCALATION_PROVIDER=gemma31` makes Gemma 4 31B available only when the host selects the escalated tier. Both model routes traverse the general 60-requests-per-minute `API_RATE_LIMITER` `platform` bucket. The topic route alone then traverses the five-requests-per-minute `MODEL_RATE_LIMITER` under its `model-topics` scope and reserves the D1 aggregate daily provider budget; the default ceiling is 100 external calls per UTC day. The offline judge makes neither a provider call nor a D1 budget reservation. A missing key or binding, invalid selector, unavailable budget/database, or provider failure uses deterministic topics; status reports invalid or incomplete deployment configuration as degraded. Missing consent rejects a configured external request before budget reservation or provider contact. There is one external attempt at most and no provider retry or Queue in this slice.

## Privacy and AI

Classic play does not require an account, an API key, transcription, or audio upload. The browser uses its microphone locally for voice-activity detection. The Cloudflare selector is shared by Practice and the current Play driver: it keeps one opaque device ID in origin-local `localStorage`, keeps labels only in page memory, and falls back once to browser Auto-detect if a saved input disappears. Selecting or listing an input sends nothing to the application API, D1, Analytics Engine, Durable Objects, or a model. The current driver can independently turn short synthesized game cues off; that preference stays in `localStorage`, no audio file is fetched, and changing it sends no application request. Coaching similarly processes microphone frames in the browser and never uploads audio or captured transcript text. With cloud summary backup off—the default—a coaching attempt makes no coaching-data API call. If the user explicitly enables backup, the app sends only the compact allowlisted summary to the NonStopTalk Worker/D1 platform. Attempt-recording/captured-transcript retention is a separate, off-by-default local-storage option; without it, audio is reduced to measurements and captured transcript text is discarded.

The Cloudflare multiplayer judge is disabled by default. If the host offers it, the exact speaker must choose again on every turn between classic play and on-device transcription; the manual timer always chooses classic. The official client requires strict `SpeechRecognition.processLocally` support and the selected live microphone track. At turn end it places no more than 8 KiB of transcript text under one 15-second in-memory delivery deadline, uses the same abort signal for the prerequisite classic-score submission and one same-origin judge request, and aborts on room/page teardown. Microphone audio is not uploaded by this flow. The Worker grades with the deterministic offline heuristic in request memory, bounds each internal room call to five seconds, and never writes transcript text to room/Durable Object state, D1, logs, analytics, history, or any provider. Classic scoring lands first and its baseline is never removed. A review that does not commit before its deadline adds no bonus; if a completion response is lost after commit, authoritative room state confirms whether that one-time bonus was applied. While a review is pending, standings and the winner are provisional. This slice needs no paid API, model key, new secret, binding, database migration, or external call. A Cloudflare judge using GLM-5.3 Flash, direct GLM-4.7 Flash, or Gemma 4 31B remains later work and must be enabled only behind a new schema/consent gate; the current model adapters are topic-generation-only.

This privacy-first judge has a casual party-game trust model, not transcript attestation or anti-cheat. The Worker proves that the requesting browser token belongs to the exact speaker and pending turn, but—because the official flow uploads no audio—it cannot prove that text from a modified client came from that live microphone. A modified client can fabricate keyword-rich text; the host's existing score correction is the recovery control.

The optional Cloudflare topic generator can make a single external request only after the host consents for that generation attempt; the normalized theme, capped at 200 characters, is the only host or room content sent to the provider. Audio, transcript text, names, and room tokens are excluded. Deterministic topic generation remains the default and fallback. The separate coaching path may create a transcript only after explicit consent and only when the browser supports mandatory on-device recognition. Derived filler/repetition patterns are saved with its compact summary and may be part of an explicitly selected cloud backup; captured transcript text is never part of that backup. By default captured transcript text is discarded; if the user separately enables full-session retention, the recording and available captured transcript are stored only for that site origin and browser profile. A finalization warning is persisted and shown whenever retained text may be partial. Coaching-card retrieval and deterministic review assembly also stay in the browser.

Cloudflare setup kits are a separate browser-local convenience, not an account or cloud library. Kit names, settings, and custom topics are stored unencrypted in `localStorage` for the current origin and browser profile, with a maximum of 25 kits, 40 Unicode code points per name, 500 topics of 200 code points each, 20,000 characters of editor text, 64 KiB per imported topic file, and 512 KiB for the serialized kit store. Saving/deleting a kit and importing/exporting topic text make no application API, model, analytics, D1, or Durable Object request and add no Cloudflare resource or cost. Import fills only the editor draft until the host explicitly uses the custom list. Applying a kit sends exactly its selected room settings/topics through one existing same-origin room action; the room Durable Object validates custom topics, replaces built-in packs with server-canonical content, and commits one atomic room version/generation change. Kit names never enter room state, D1, Analytics Engine, or a model, and guests never receive the undrawn topic list. Browser storage is best effort, unsynced, and has no recovery; exported `.txt` files are outside the app's deletion or retention control. Deleting coaching history does not delete game setup kits.

The local Go AI judge is separately opt-in at two levels: the host enables it for the room, then the current speaker chooses whether to use transcription for that turn. Transcription starts only when the browser exposes `SpeechRecognition` with `processLocally` support and accepts the selected live microphone track. If any of those checks fail, the turn continues with classic scoring or the manual timer.
...
The optional judge adds `round(relevance × 20)` points. Classic scoring lands first. Judge work is asynchronous and best-effort, so a timeout, missing transcript, provider or offline-judge error, or interrupted restore leaves the classic score intact. The host can adjust any player's total in five-point increments.
...
`test:cloudflare` and the Go test suite both consume `testdata/game-contract.v1.json`, which locks six core rule families to the same expected behavior in both editions: Unicode limits, remote-clock tolerance, completion normalization, score corrections, atomic custom topics, and persisted turn-ID repair. Both runtimes also consume `testdata/judge-contract.v1.json` for the offline grading formula, feedback, bonus, and confidence-label contract. `test:turn-transcription` isolates strict on-device capability, selected-track use, transcript normalization/limit, stale-turn cleanup, memory clearing, and the bounded abortable handoff across classic submission and judge delivery. Cloudflare tests additionally cover the same-origin judge route, transcript-free claim lifecycle, exact-speaker authorization, fail-closed resolution, and provisional standings. They also cover canonical milestone payloads, receipt replay/conflict behavior, the isolated post-commit Analytics Engine attempt, schema-6 receipt expiry, the absence of receipt SQL from schema-5 cleanup, and the Release-A rollback bridge. A separate exact-outbox Workers runtime proves atomic room/event/alarm commits, deterministic transaction replay, FIFO lifecycle delivery, marker-5 recovery, eviction and concurrent joins, expected fail-open drops, second-event and alarm rollback, capacity boundaries, exactly-one routing, and both Worker/Durable-Object propagation-skew directions; the original runtime remains a deliberate best-effort compatibility control that proves a non-exact room creates no outbox. A bounded wrapper rejects known teardown, pending-RPC, uncaught-runtime, and other false-green signatures even when Vitest exits zero. `test:coach` runs 46 deterministic tests: 38 measurement, calibration-readiness, continuity, transcript, retrieval, advice, relationship, and comparison tests plus eight IndexedDB storage-contract tests. `test:microphone-selection` adds 23 dependency-injected storage, bounds, constraint, fallback, cancellation, refresh-order, and stream-cleanup tests. `test:sound-cues` adds 34 dependency-injected preference, migration, exact-recipe, Web Audio failure, stale-resume, cleanup, and deferred judge-transition tests. `smoke:coach` drives a persisted exact input through selection and calibration before continuing through the single coached path and default review-only baseline → Progress/reload → locked unassisted-retry path. It covers medium/high calibration fast paths; the low-confidence retry/continue/cancel gate; race-safe controls; keyboard operation; 320-pixel layout; the Review confidence explanation; and the rule that recording/transcription cannot start while the gate is waiting. It checks that input choice, local-first attempts, and the calibration gate make no coaching-data API request, and also covers UI-integrated local storage, the exact point-in-time artifact dashboard, per-attempt artifact deletion, v1→v3 opening, active-v3 use of a compatible future schema, lifecycle expiry, app-limit/browser-quota summary-only messages, composed warnings, and connection closure before a later upgrade. `test:cloud-progress` checks the separate opt-in client's summary allowlist, relationship metadata, legacy compatibility, merge behavior, API calls, and preference state. `test:admin` validates dashboard source reconciliation, UTC windows, request/token boundaries, empty data, and hostile display text. `smoke:admin` exercises the isolated document, strict headers, two-request authorization flow, 320px layout, keyboard focus, and absence of third-party requests or persistence. `smoke:accessibility` runs automated axe WCAG-rule scans plus heading, dialog/focus, title, skip-link, reduced-motion, sound-preference keyboard semantics, and narrow-layout assertions across the public routes and core room states; it is regression coverage, not formal conformance certification. `smoke:multiplayer` boots a hermetic local Worker/D1/Durable Object and drives two isolated browser identities through WebSocket propagation, host authorization, driver-only input choice and sound cues, exact capture-before-begin ordering, successful-action cue ordering, authority-loss cleanup, optional offline judging, scoring, reconnect persistence, and a finished game. Its verified process-tree cleanup requires Linux or macOS; use WSL or the Ubuntu CI job instead of native Windows. `test:production-monitor` locks the scheduled read-only health workflow to production and staging `durable-outbox` policy, recursively checks all 12 deployable `.js`/`.mjs` Static Assets as one exact canonical UTF-8/no-BOM release generation, verifies that all nine excluded `*.test.js`/`*.test.mjs` paths are not executable, and prevents secrets, mutation commands, installs, unpinned actions, or unbounded runtime. `test:production-outbox-smoke` validates the completed activation canary's target locks, rollback-version contract, resource parity, quiet-window delta checks, and refusal to run under automation without touching production. `smoke:platform` starts a local Wrangler/D1 instance and exercises status, backup, relationship-column persistence, export, analytics, privacy, and deletion boundaries.

`smoke:coach-storage` is the dedicated lifecycle proof: it serves the hash-pinned Release-A fixture and current module on one browser origin, then verifies atomic v2→v3 migration, exact 30-day/byte/cap boundaries, fail-closed corruption, no-eviction and two-tab behavior, blocked/aborted upgrades, and Release A → v3 → Release A → v3 restoration. Preview URLs on different origins cannot prove that handoff.
...
- A pure cross-runtime offline-judge contract plus a same-origin Cloudflare judge route; the browser owns consent/transcription, the Worker handles transcript only in request memory, and the Durable Object stores only transcript-free review metadata and verdicts
...
- Origin-scoped IndexedDB v3 for compact coaching summaries—including explicit loop/baseline/role metadata—and a separate, explicitly opted-in recording/captured-transcript store. A required content-free ledger atomically tracks retention and logical bytes; new and migrated artifacts receive an exact 30-day window, and new artifacts share a 128 MiB logical cap without eviction. A single streamed reconciliation supplies Progress with exact local artifact usage and per-attempt deadlines without loading the whole Blob store or estimating browser capacity. App-limit or browser-quota outcomes preserve the compact summary only. Incompatible storage fails closed, while the deployed Release-A floor can reopen a v3-or-newer compatible database without attempting a downgrade. Exports exclude artifacts, per-attempt deletion preserves the compact record, and full local deletion clears all local coaching stores
- An in-progress modular platform layer using D1 for queryable consented summaries and protected daily rollups, plus Analytics Engine for coarse best-effort telemetry; Durable Objects remain responsible for live rooms, and the configuration-gated atomic outbox producer is selected by the committed production and staging policy, with a Release-A-compatible consumer and no additional service
...
- Remote-provider judge parity between the local Go and free Cloudflare editions; the Cloudflare offline relevance judge is implemented, while any GLM/Gemma judge path remains schema-gated future work
